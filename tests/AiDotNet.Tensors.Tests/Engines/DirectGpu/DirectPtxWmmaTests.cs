...
public class DirectPtxWmmaTests
{
    ...
    {
        Assert.Equal(14, DirectPtxQuantizedMixedSparseCoverageManifest.All.Count);
        ...
        int outputFeatures)
    {
        ...
        float[] canonicalWeights = Values(random, inputFeatures * outputFeatures, 0.0625f);
        float[] weightsHost = canonicalWeights;
        float[] biasHost = Values(random, outputFeatures, 0.0625f);
        float[] expected = FusedLinearGeluOracle(
            inputHost, canonicalWeights, biasHost, inputFeatures, outputFeatures);
    ...
    public void BackendFusedLinearGeluM1_IsExactZeroAllocationCapturableAndPublic()
    {
        ...
        try
        {
            ...
            Assert.True(allLaunched, backend.DirectPtxLastError);
            Assert.Equal(0, allocated);
            AssertVectorClose(backend.DownloadBuffer(output), expected, 2e-4f, "backend fused linear GELU");
            ...
            AssertVectorClose(backend.DownloadBuffer(output), expected, 2e-4f, "public fused linear GELU");
        }
        finally
        {
            DirectPtxFeatureGate.TestOverride = previous;
