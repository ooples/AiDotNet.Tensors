...
public sealed class DirectPtxConvolutionTests
{
    ...
    [Fact]
    public void ConvTranspose3D3x3x3Emitter_IsNegatedHaloChannelLoopSm86Ptx()
    {
        string ptx = PtxConvTranspose3DNcdhw3x3x3ForwardF32Kernel.EmitPtx(8, 6);

        Assert.Contains(".target sm_86", ptx, StringComparison.Ordinal);
        Assert.Contains(PtxConvTranspose3DNcdhw3x3x3ForwardF32Kernel.EntryPoint, ptx, StringComparison.Ordinal);
        Assert.Equal(3, Count(ptx, ".param .u64"));
        Assert.Contains("CONVT3D_CHANNELS:", ptx, StringComparison.Ordinal);
        // 27 taps unrolled per input channel.
        Assert.Equal(27, Count(ptx, "fma.rn.f32"));
        Assert.Contains("@%p7 bra CONVT3D_CHANNELS", ptx, StringComparison.Ordinal);
        Assert.Equal(1, Count(ptx, "st.global.f32"));
        Assert.DoesNotContain(".shared", ptx, StringComparison.Ordinal);
        Assert.DoesNotContain(".param .u32", ptx, StringComparison.Ordinal);
        Assert.Throws<NotSupportedException>(() =>
            PtxConvTranspose3DNcdhw3x3x3ForwardF32Kernel.EmitPtx(8, 9));
    }

    [Fact]
    public void ConvTranspose3D3x3x3ManifestCell_IsExperimentalWithDedicatedRoute()
    {
        Assert.Equal(DirectPtxConvolutionCoverageStatus.ExperimentalDirectPtx,
            DirectPtxConvolutionCoverageManifest.Get("IEngine.ConvTranspose3D").Status);
        Assert.Equal(DirectPtxConvolutionCoverageStatus.ExperimentalDirectPtx,
            DirectPtxConvolutionCoverageManifest
                .Get("CudaBackend.TryDirectPtxConvTranspose3D3x3x3").Status);
    }

    [SkippableFact]
    public void DriverOnlyConvTranspose3D3x3x3_MatchesCpuReference()
    {
        Skip.IfNot(DirectPtxRuntime.IsAvailable, "Requires an NVIDIA CUDA driver and GPU.");
        using var runtime = new DirectPtxRuntime();
        Skip.IfNot(DirectPtxArchitecture.HasExperimentalConvolution(
        ...
        const int inChannels = PtxConvTranspose3DNcdhw3x3x3ForwardF32Kernel.InputChannels;
        const int outChannels = PtxConvTranspose3DNcdhw3x3x3ForwardF32Kernel.OutputChannels;
        const int depth = PtxConvTranspose3DNcdhw3x3x3ForwardF32Kernel.Depth;
        const int hgt = PtxConvTranspose3DNcdhw3x3x3ForwardF32Kernel.Height;
        const int wid = PtxConvTranspose3DNcdhw3x3x3ForwardF32Kernel.Width;
        const int kernelSize = PtxConvTranspose3DNcdhw3x3x3ForwardF32Kernel.KernelSize;
        int spatial = depth * hgt * wid;
        int hw = hgt * wid;

        using var kernel = new PtxConvTranspose3DNcdhw3x3x3ForwardF32Kernel(runtime);
        using var inputDevice = runtime.AllocateBytes((nuint)PtxConvTranspose3DNcdhw3x3x3ForwardF32Kernel.InputBytes);
        using var weightDevice = runtime.AllocateBytes((nuint)PtxConvTranspose3DNcdhw3x3x3ForwardF32Kernel.WeightBytes);
        using var outputDevice = runtime.AllocateBytes((nuint)PtxConvTranspose3DNcdhw3x3x3ForwardF32Kernel.OutputBytes);
        ...
        for (int oh = 0; oh < hgt; oh++)
        for (int ow = 0; ow < wid; ow++)
        {
            float expected = 0;
            for (int ci = 0; ci < inChannels; ci++)
            for (int kd = 0; kd < kernelSize; kd++)
            ...
            float got = actual[co * spatial + (od * hw + oh * wid + ow)];
            maxAbsoluteError = MathF.Max(maxAbsoluteError, MathF.Abs(got - expected));
        }
    ...
    [Fact]
    public void ConvTranspose3D3x3x3BackwardWeightEmitter_IsTransposedVolumeDotSm86Ptx()
    {
        string ptx = PtxConvTranspose3DNcdhw3x3x3BackwardWeightF32Kernel.EmitPtx(8, 6);

        Assert.Contains(".target sm_86", ptx, StringComparison.Ordinal);
        Assert.Contains(PtxConvTranspose3DNcdhw3x3x3BackwardWeightF32Kernel.EntryPoint, ptx, StringComparison.Ordinal);
        Assert.Equal(3, Count(ptx, ".param .u64"));
        Assert.Contains("CONVT3D_BWD_WEIGHT:", ptx, StringComparison.Ordinal);
        // Three id-decompose divides (id/27, tap/9, rem9/3).
        Assert.Equal(3, Count(ptx, "div.u32"));
        Assert.Equal(1, Count(ptx, "fma.rn.f32"));
        Assert.Equal(1, Count(ptx, "st.global.f32"));
        Assert.Contains("@%p7 bra CONVT3D_BWD_WEIGHT", ptx, StringComparison.Ordinal);
        Assert.DoesNotContain(".shared", ptx, StringComparison.Ordinal);
        Assert.DoesNotContain("red.global", ptx, StringComparison.Ordinal);
        Assert.Throws<NotSupportedException>(() =>
            PtxConvTranspose3DNcdhw3x3x3BackwardWeightF32Kernel.EmitPtx(8, 9));
    }

    [Fact]
    public void ConvTranspose3D3x3x3BackwardWeightManifestCell_IsExperimentalWithDedicatedRoute()
    {
        Assert.Equal(DirectPtxConvolutionCoverageStatus.ExperimentalDirectPtx,
            DirectPtxConvolutionCoverageManifest.Get("IEngine.ConvTranspose3DBackwardKernel").Status);
        Assert.Equal(DirectPtxConvolutionCoverageStatus.ExperimentalDirectPtx,
            DirectPtxConvolutionCoverageManifest
                .Get("CudaBackend.TryDirectPtxConvTranspose3D3x3x3BackwardWeight").Status);
    }

    [SkippableFact]
    public void DriverOnlyConvTranspose3D3x3x3BackwardWeight_MatchesCpuReference()
    {
        Skip.IfNot(DirectPtxRuntime.IsAvailable, "Requires an NVIDIA CUDA driver and GPU.");
        using var runtime = new DirectPtxRuntime();
        Skip.IfNot(DirectPtxArchitecture.HasExperimentalConvolution(
        ...
        const int inChannels = PtxConvTranspose3DNcdhw3x3x3BackwardWeightF32Kernel.InputChannels;
        const int outChannels = PtxConvTranspose3DNcdhw3x3x3BackwardWeightF32Kernel.OutputChannels;
        const int depth = PtxConvTranspose3DNcdhw3x3x3BackwardWeightF32Kernel.Depth;
        const int hgt = PtxConvTranspose3DNcdhw3x3x3BackwardWeightF32Kernel.Height;
        const int wid = PtxConvTranspose3DNcdhw3x3x3BackwardWeightF32Kernel.Width;
        const int kernelSize = PtxConvTranspose3DNcdhw3x3x3BackwardWeightF32Kernel.KernelSize;
        int spatial = depth * hgt * wid;
        int hw = hgt * wid;

        using var kernel = new PtxConvTranspose3DNcdhw3x3x3BackwardWeightF32Kernel(runtime);
        using var gradOutDevice = runtime.AllocateBytes((nuint)PtxConvTranspose3DNcdhw3x3x3BackwardWeightF32Kernel.GradOutputBytes);
        using var inputDevice = runtime.AllocateBytes((nuint)PtxConvTranspose3DNcdhw3x3x3BackwardWeightF32Kernel.InputBytes);
        using var gradWeightDevice = runtime.AllocateBytes((nuint)PtxConvTranspose3DNcdhw3x3x3BackwardWeightF32Kernel.GradWeightBytes);
        ...
        kernel.Launch(
            ...
            DirectPtxTensorView.CreateOwned(gradWeightDevice, kernel.Blueprint.Tensors[2]));
        runtime.Synchronize();
        var actual = new float[inChannels * outChannels * kernelSize * kernelSize * kernelSize];
        gradWeightDevice.Download<float>(actual);
        ...
        for (int kw = 0; kw < kernelSize; kw++)
        {
            ...
            for (int ow = 0; ow < wid; ow++)
            {
                int id = od + 1 - kd, ih = oh + 1 - kh, iw = ow + 1 - kw;
                if (id < 0 || id >= depth || ih < 0 || ih >= hgt || iw < 0 || iw >= wid) continue;
                expected += gradOut[co * spatial + (od * hw + oh * wid + ow)] *
                    input[ci * spatial + (id * hw + ih * wid + iw)];
            }
            int wIdx = (((ci * outChannels + co) * kernelSize + kd) * kernelSize + kh) * kernelSize + kw;
            maxAbsoluteError = MathF.Max(maxAbsoluteError, MathF.Abs(actual[wIdx] - expected));
    ...
    [Fact]
    public void UnfoldKNFp16FromFp16Emitter_IsVerbatimFp16GatherSm86Ptx()
    {
        string ptx = PtxUnfoldKNFp16FromFp16Nchw3x3Kernel.EmitPtx(8, 6);

        Assert.Contains(".target sm_86", ptx, StringComparison.Ordinal);
        Assert.Contains(PtxUnfoldKNFp16FromFp16Nchw3x3Kernel.EntryPoint, ptx, StringComparison.Ordinal);
        Assert.Equal(2, Count(ptx, ".param .u64"));
        // Pure two-byte gather: FP16 load and store, no numeric conversion.
        Assert.Equal(2, Count(ptx, "div.u32"));
        Assert.Equal(1, Count(ptx, "ld.global.nc.b16"));
        Assert.Equal(1, Count(ptx, "st.global.b16"));
        Assert.Equal(0, Count(ptx, "cvt.rn.f16.f32"));
        Assert.DoesNotContain(".f32", ptx, StringComparison.Ordinal);
        Assert.DoesNotContain(".shared", ptx, StringComparison.Ordinal);
        Assert.DoesNotContain("atom", ptx, StringComparison.Ordinal);
        Assert.Throws<NotSupportedException>(() =>
            PtxUnfoldKNFp16FromFp16Nchw3x3Kernel.EmitPtx(8, 9));
    }

    [Fact]
    public void UnfoldKNFp16FromFp16ManifestCell_IsExperimentalWithDedicatedRoute()
    {
        Assert.Equal(DirectPtxConvolutionCoverageStatus.ExperimentalDirectPtx,
            DirectPtxConvolutionCoverageManifest.Get("CudaBackend.UnfoldKNFp16FromFp16").Status);
        Assert.Equal(DirectPtxConvolutionCoverageStatus.ExperimentalDirectPtx,
            DirectPtxConvolutionCoverageManifest
                .Get("CudaBackend.TryDirectPtxUnfoldKNFp16FromFp16").Status);
    }

    [SkippableFact]
    public void DriverOnlyUnfoldKNFp16FromFp16_MatchesCpuReference()
    {
        Skip.IfNot(DirectPtxRuntime.IsAvailable, "Requires an NVIDIA CUDA driver and GPU.");
        using var runtime = new DirectPtxRuntime();
        Skip.IfNot(DirectPtxArchitecture.HasExperimentalConvolution(
        ...
        const int inChannels = PtxUnfoldKNFp16FromFp16Nchw3x3Kernel.InputChannels;
        const int height = PtxUnfoldKNFp16FromFp16Nchw3x3Kernel.Height;
        const int width = PtxUnfoldKNFp16FromFp16Nchw3x3Kernel.Width;
        const int kernelSize = PtxUnfoldKNFp16FromFp16Nchw3x3Kernel.KernelSize;
        const int patchDim = PtxUnfoldKNFp16FromFp16Nchw3x3Kernel.PatchDim;   // K
        const int n = PtxUnfoldKNFp16FromFp16Nchw3x3Kernel.SpatialElements;   // N
        int spatial = height * width;

        using var kernel = new PtxUnfoldKNFp16FromFp16Nchw3x3Kernel(runtime);
        using var inputDevice = runtime.AllocateBytes((nuint)PtxUnfoldKNFp16FromFp16Nchw3x3Kernel.InputBytes);
        using var outputDevice = runtime.AllocateBytes((nuint)PtxUnfoldKNFp16FromFp16Nchw3x3Kernel.OutputBytes);

        Random random = AiDotNet.Tensors.Helpers.RandomHelper.CreateSecureRandom();
        var inputBits = new ushort[inChannels * spatial];
        for (int i = 0; i < inputBits.Length; i++)
            inputBits[i] = BitConverter.HalfToUInt16Bits((Half)(random.NextDouble() * 2 - 1));
        inputDevice.Upload<ushort>(inputBits);
        ...
        for (int k = 0; k < patchDim; k++)
        {
            ...
            for (int col = 0; col < n; col++)
            {
                ...
                int iy = oy + ky - 1;
                int ix = ox + kx - 1;
                ushort expected = (iy < 0 || iy >= height || ix < 0 || ix >= width)
                    ? (ushort)0
                    : inputBits[ci * spatial + (iy * width + ix)];
                if (actual[k * n + col] != expected) mismatches++;
            }
        }
    ...
    [Fact]
    public void DeformableConv2DGroupedEmitter_IsPerGroupBilinearSm86Ptx()
    {
        string ptx = PtxDeformableConv2DGroupedNchw3x3F32Kernel.EmitPtx(8, 6);

        Assert.Contains(".target sm_86", ptx, StringComparison.Ordinal);
        Assert.Contains(PtxDeformableConv2DGroupedNchw3x3F32Kernel.EntryPoint, ptx, StringComparison.Ordinal);
        Assert.Equal(5, Count(ptx, ".param .u64"));
        Assert.Contains("DEFORM_GROUPED_CI:", ptx, StringComparison.Ordinal);
        // One group divide per input-channel iteration; 9 taps each recompute bilinear (18 floors).
        Assert.Equal(1, Count(ptx, "div.u32"));
        Assert.Equal(18, Count(ptx, "cvt.rmi.s32.f32"));
        Assert.Contains("@%p12 bra DEFORM_GROUPED_CI", ptx, StringComparison.Ordinal);
        Assert.Equal(1, Count(ptx, "st.global.f32"));
        Assert.DoesNotContain(".shared", ptx, StringComparison.Ordinal);
        Assert.DoesNotContain("atom", ptx, StringComparison.Ordinal);
        Assert.Throws<NotSupportedException>(() =>
            PtxDeformableConv2DGroupedNchw3x3F32Kernel.EmitPtx(8, 9));
    }

    [Fact]
    public void DeformableConv2DGroupedManifestCell_IsExperimentalWithDedicatedRoute()
    {
        Assert.Equal(DirectPtxConvolutionCoverageStatus.ExperimentalDirectPtx,
            DirectPtxConvolutionCoverageManifest.Get("IEngine.DeformableConv2DGrouped").Status);
        Assert.Equal(DirectPtxConvolutionCoverageStatus.ExperimentalDirectPtx,
            DirectPtxConvolutionCoverageManifest
                .Get("CudaBackend.TryDirectPtxDeformableConv2DGrouped").Status);
    }

    [SkippableFact]
    public void DriverOnlyDeformableConv2DGrouped_MatchesCpuReference()
    {
        Skip.IfNot(DirectPtxRuntime.IsAvailable, "Requires an NVIDIA CUDA driver and GPU.");
        using var runtime = new DirectPtxRuntime();
        Skip.IfNot(DirectPtxArchitecture.HasExperimentalConvolution(
        ...
        const int inChannels = PtxDeformableConv2DGroupedNchw3x3F32Kernel.InputChannels;
        const int outChannels = PtxDeformableConv2DGroupedNchw3x3F32Kernel.OutputChannels;
        const int height = PtxDeformableConv2DGroupedNchw3x3F32Kernel.Height;
        const int width = PtxDeformableConv2DGroupedNchw3x3F32Kernel.Width;
        const int kernelSize = PtxDeformableConv2DGroupedNchw3x3F32Kernel.KernelSize;
        const int taps = PtxDeformableConv2DGroupedNchw3x3F32Kernel.TapsPerChannel;
        const int channelsPerGroup = PtxDeformableConv2DGroupedNchw3x3F32Kernel.ChannelsPerGroup;
        const int offChannels = PtxDeformableConv2DGroupedNchw3x3F32Kernel.OffsetChannels;
        const int maskChannels = PtxDeformableConv2DGroupedNchw3x3F32Kernel.MaskChannels;
        int spatial = height * width;

        using var kernel = new PtxDeformableConv2DGroupedNchw3x3F32Kernel(runtime);
        using var inputDevice = runtime.AllocateBytes((nuint)PtxDeformableConv2DGroupedNchw3x3F32Kernel.InputBytes);
        using var weightDevice = runtime.AllocateBytes((nuint)PtxDeformableConv2DGroupedNchw3x3F32Kernel.WeightBytes);
        using var offsetDevice = runtime.AllocateBytes((nuint)PtxDeformableConv2DGroupedNchw3x3F32Kernel.OffsetBytes);
        using var maskDevice = runtime.AllocateBytes((nuint)PtxDeformableConv2DGroupedNchw3x3F32Kernel.MaskBytes);
        using var outputDevice = runtime.AllocateBytes((nuint)PtxDeformableConv2DGroupedNchw3x3F32Kernel.OutputBytes);
        ...
        for (int ox = 0; ox < width; ox++)
        {
            ...
            for (int ci = 0; ci < inChannels; ci++)
            {
                ...
                for (int kx = 0; kx < kernelSize; kx++)
                {
                    ...
                    float py = oy + ky - 1 + offY;
                    float px = ox + kx - 1 + offX;
                    float m = mask[(g * taps + t) * spatial + s];
                    float sample = Bilinear(input, ci, spatial, height, width, py, px);
                    acc += weights[((co * inChannels + ci) * kernelSize + ky) * kernelSize + kx] * m * sample;
                }
            }
    ...
    [Fact]
    public void DeformableConv2DGroupedBackwardMaskEmitter_IsPerGroupInGroupReductionSm86Ptx()
    {
        string ptx = PtxDeformableConv2DGroupedNchw3x3BackwardMaskF32Kernel.EmitPtx(8, 6);

        Assert.Contains(".target sm_86", ptx, StringComparison.Ordinal);
        Assert.Contains(PtxDeformableConv2DGroupedNchw3x3BackwardMaskF32Kernel.EntryPoint, ptx, StringComparison.Ordinal);
        Assert.Equal(5, Count(ptx, ".param .u64"));
        // Two divides decompose the mask channel (gm/9) and tap (t/3); bilinear once.
        Assert.Equal(2, Count(ptx, "div.u32"));
        Assert.Equal(2, Count(ptx, "cvt.rmi.s32.f32"));
        Assert.Contains("DEFORM_GROUPED_BWD_MASK_J:", ptx, StringComparison.Ordinal);
        Assert.Contains("DEFORM_GROUPED_BWD_MASK_CO:", ptx, StringComparison.Ordinal);
        Assert.Contains("@%p13 bra DEFORM_GROUPED_BWD_MASK_J", ptx, StringComparison.Ordinal);
        Assert.Equal(1, Count(ptx, "st.global.f32"));
        Assert.DoesNotContain(".shared", ptx, StringComparison.Ordinal);
        Assert.DoesNotContain("atom", ptx, StringComparison.Ordinal);
        Assert.Throws<NotSupportedException>(() =>
            PtxDeformableConv2DGroupedNchw3x3BackwardMaskF32Kernel.EmitPtx(8, 9));
    }

    [Fact]
    public void DeformableConv2DGroupedBackwardMaskManifestCell_IsExperimentalWithDedicatedRoute()
    {
        Assert.Equal(DirectPtxConvolutionCoverageStatus.ExperimentalDirectPtx,
            DirectPtxConvolutionCoverageManifest.Get("IEngine.DeformableConv2DGroupedBackward").Status);
        Assert.Equal(DirectPtxConvolutionCoverageStatus.ExperimentalDirectPtx,
            DirectPtxConvolutionCoverageManifest
                .Get("CudaBackend.TryDirectPtxDeformableConv2DGroupedBackwardMask").Status);
    }

    [SkippableFact]
    public void DriverOnlyDeformableConv2DGroupedBackwardMask_MatchesCpuReference()
    {
        Skip.IfNot(DirectPtxRuntime.IsAvailable, "Requires an NVIDIA CUDA driver and GPU.");
        using var runtime = new DirectPtxRuntime();
        Skip.IfNot(DirectPtxArchitecture.HasExperimentalConvolution(
        ...
        const int inChannels = PtxDeformableConv2DGroupedNchw3x3BackwardMaskF32Kernel.InputChannels;
        const int outChannels = PtxDeformableConv2DGroupedNchw3x3BackwardMaskF32Kernel.OutputChannels;
        const int height = PtxDeformableConv2DGroupedNchw3x3BackwardMaskF32Kernel.Height;
        const int width = PtxDeformableConv2DGroupedNchw3x3BackwardMaskF32Kernel.Width;
        const int kernelSize = PtxDeformableConv2DGroupedNchw3x3BackwardMaskF32Kernel.KernelSize;
        const int taps = PtxDeformableConv2DGroupedNchw3x3BackwardMaskF32Kernel.TapsPerChannel;
        const int channelsPerGroup = PtxDeformableConv2DGroupedNchw3x3BackwardMaskF32Kernel.ChannelsPerGroup;
        const int offChannels = PtxDeformableConv2DGroupedNchw3x3BackwardMaskF32Kernel.OffsetChannels;
        const int maskChannels = PtxDeformableConv2DGroupedNchw3x3BackwardMaskF32Kernel.MaskChannels;
        int spatial = height * width;

        using var kernel = new PtxDeformableConv2DGroupedNchw3x3BackwardMaskF32Kernel(runtime);
        using var gradOutDevice = runtime.AllocateBytes((nuint)PtxDeformableConv2DGroupedNchw3x3BackwardMaskF32Kernel.GradOutputBytes);
        using var inputDevice = runtime.AllocateBytes((nuint)PtxDeformableConv2DGroupedNchw3x3BackwardMaskF32Kernel.InputBytes);
        using var weightDevice = runtime.AllocateBytes((nuint)PtxDeformableConv2DGroupedNchw3x3BackwardMaskF32Kernel.WeightBytes);
        using var offsetDevice = runtime.AllocateBytes((nuint)PtxDeformableConv2DGroupedNchw3x3BackwardMaskF32Kernel.OffsetBytes);
        using var gradMaskDevice = runtime.AllocateBytes((nuint)PtxDeformableConv2DGroupedNchw3x3BackwardMaskF32Kernel.GradMaskBytes);
        ...
        kernel.Launch(
            ...
            DirectPtxTensorView.CreateOwned(gradMaskDevice, kernel.Blueprint.Tensors[4]));
        runtime.Synchronize();
        var actual = new float[maskChannels * spatial];
        gradMaskDevice.Download<float>(actual);
        ...
        float maxAbsoluteError = 0;
        for (int g = 0; g < maskChannels / taps; g++)
        for (int t = 0; t < taps; t++)
        {
            int ky = t / kernelSize;
            int kx = t % kernelSize;
            ...
            for (int ox = 0; ox < width; ox++)
            {
                ...
                {
                    int ci = g * channelsPerGroup + j;
                    float sample = Bilinear(input, ci, spatial, height, width, py, px);
                    for (int co = 0; co < outChannels; co++)
                        expected += gradOut[co * spatial + s]
                            * weights[((co * inChannels + ci) * kernelSize + ky) * kernelSize + kx] * sample;
                }
                maxAbsoluteError = MathF.Max(maxAbsoluteError, MathF.Abs(actual[gm * spatial + s] - expected));
            }
        }

        Assert.True(maxAbsoluteError <= 2e-4f,
            $"Grouped deformable Conv2D backward-mask max absolute error {maxAbsoluteError:G9}.");
    }

    [Fact]
    public void DeformableConv2DGroupedBackwardOffsetEmitter_IsPerGroupSpatialDerivativeSm86Ptx()
    {
        string ptx = PtxDeformableConv2DGroupedNchw3x3BackwardOffsetF32Kernel.EmitPtx(8, 6);

        Assert.Contains(".target sm_86", ptx, StringComparison.Ordinal);
        Assert.Contains(PtxDeformableConv2DGroupedNchw3x3BackwardOffsetF32Kernel.EntryPoint, ptx, StringComparison.Ordinal);
        Assert.Equal(6, Count(ptx, ".param .u64"));
        Assert.Equal(2, Count(ptx, "div.u32"));
        Assert.Equal(2, Count(ptx, "cvt.rmi.s32.f32"));
        Assert.Contains("DEFORM_GROUPED_BWD_OFFSET_J:", ptx, StringComparison.Ordinal);
        Assert.Contains("DEFORM_GROUPED_BWD_OFFSET_CO:", ptx, StringComparison.Ordinal);
        // Dual outputs dOffY and dOffX.
        Assert.Equal(2, Count(ptx, "st.global.f32"));
        Assert.DoesNotContain(".shared", ptx, StringComparison.Ordinal);
        Assert.DoesNotContain("atom", ptx, StringComparison.Ordinal);
        Assert.Throws<NotSupportedException>(() =>
            PtxDeformableConv2DGroupedNchw3x3BackwardOffsetF32Kernel.EmitPtx(8, 9));
    }

    [Fact]
    public void DeformableConv2DGroupedBackwardOffsetManifestCell_IsExperimentalWithDedicatedRoute()
    {
        Assert.Equal(DirectPtxConvolutionCoverageStatus.ExperimentalDirectPtx,
            DirectPtxConvolutionCoverageManifest.Get("IEngine.DeformableConv2DGroupedBackward").Status);
        Assert.Equal(DirectPtxConvolutionCoverageStatus.ExperimentalDirectPtx,
            DirectPtxConvolutionCoverageManifest
                .Get("CudaBackend.TryDirectPtxDeformableConv2DGroupedBackwardOffset").Status);
    }

    [SkippableFact]
    public void DriverOnlyDeformableConv2DGroupedBackwardOffset_MatchesCpuReference()
    {
        Skip.IfNot(DirectPtxRuntime.IsAvailable, "Requires an NVIDIA CUDA driver and GPU.");
        using var runtime = new DirectPtxRuntime();
        Skip.IfNot(DirectPtxArchitecture.HasExperimentalConvolution(
        ...
        const int inChannels = PtxDeformableConv2DGroupedNchw3x3BackwardOffsetF32Kernel.InputChannels;
        const int outChannels = PtxDeformableConv2DGroupedNchw3x3BackwardOffsetF32Kernel.OutputChannels;
        const int height = PtxDeformableConv2DGroupedNchw3x3BackwardOffsetF32Kernel.Height;
        const int width = PtxDeformableConv2DGroupedNchw3x3BackwardOffsetF32Kernel.Width;
        const int kernelSize = PtxDeformableConv2DGroupedNchw3x3BackwardOffsetF32Kernel.KernelSize;
        const int taps = PtxDeformableConv2DGroupedNchw3x3BackwardOffsetF32Kernel.TapsPerChannel;
        const int channelsPerGroup = PtxDeformableConv2DGroupedNchw3x3BackwardOffsetF32Kernel.ChannelsPerGroup;
        const int offChannels = PtxDeformableConv2DGroupedNchw3x3BackwardOffsetF32Kernel.OffsetChannels;
        const int maskChannels = PtxDeformableConv2DGroupedNchw3x3BackwardOffsetF32Kernel.MaskChannels;
        int spatial = height * width;

        using var kernel = new PtxDeformableConv2DGroupedNchw3x3BackwardOffsetF32Kernel(runtime);
        using var gradOutDevice = runtime.AllocateBytes((nuint)PtxDeformableConv2DGroupedNchw3x3BackwardOffsetF32Kernel.GradOutputBytes);
        using var inputDevice = runtime.AllocateBytes((nuint)PtxDeformableConv2DGroupedNchw3x3BackwardOffsetF32Kernel.InputBytes);
        using var weightDevice = runtime.AllocateBytes((nuint)PtxDeformableConv2DGroupedNchw3x3BackwardOffsetF32Kernel.WeightBytes);
        using var offsetDevice = runtime.AllocateBytes((nuint)PtxDeformableConv2DGroupedNchw3x3BackwardOffsetF32Kernel.OffsetBytes);
        using var maskDevice = runtime.AllocateBytes((nuint)PtxDeformableConv2DGroupedNchw3x3BackwardOffsetF32Kernel.MaskBytes);
        using var gradOffsetDevice = runtime.AllocateBytes((nuint)PtxDeformableConv2DGroupedNchw3x3BackwardOffsetF32Kernel.GradOffsetBytes);
        ...
        kernel.Launch(
            ...
            DirectPtxTensorView.CreateOwned(gradOffsetDevice, kernel.Blueprint.Tensors[5]));
        runtime.Synchronize();
        var actual = new float[offChannels * spatial];
        gradOffsetDevice.Download<float>(actual);
        ...
        for (int t = 0; t < taps; t++)
        {
            ...
            for (int ox = 0; ox < width; ox++)
            {
                ...
                for (int j = 0; j < channelsPerGroup; j++)
                {
                    ...
                    float gsum = 0;
                    for (int co = 0; co < outChannels; co++)
                        gsum += gradOut[co * spatial + s] * weights[((co * inChannels + ci) * kernelSize + ky) * kernelSize + kx];
                    accY += gsum * dpy;
                    accX += gsum * dpx;
                }
                float m = mask[(g * taps + t) * spatial + s];
                maxAbsoluteError = MathF.Max(maxAbsoluteError, MathF.Abs(actual[(g * 2 * taps + 2 * t) * spatial + s] - m * accY));
                maxAbsoluteError = MathF.Max(maxAbsoluteError, MathF.Abs(actual[(g * 2 * taps + 2 * t + 1) * spatial + s] - m * accX));
            }
        }

        Assert.True(maxAbsoluteError <= 3e-4f,
            $"Grouped deformable Conv2D backward-offset max absolute error {maxAbsoluteError:G9}.");
    }

    [Fact]
    public void DeformableConv2DGroupedBackwardInputEmitter_IsPerGroupTransposeGatherSm86Ptx()
    {
        string ptx = PtxDeformableConv2DGroupedNchw3x3BackwardInputF32Kernel.EmitPtx(8, 6);

        Assert.Contains(".target sm_86", ptx, StringComparison.Ordinal);
        Assert.Contains(PtxDeformableConv2DGroupedNchw3x3BackwardInputF32Kernel.EntryPoint, ptx, StringComparison.Ordinal);
        Assert.Equal(5, Count(ptx, ".param .u64"));
        // g(d)=max(0,1-|d|): two abs + two max; triple loop t -> s -> co.
        Assert.Equal(2, Count(ptx, "abs.f32"));
        Assert.Equal(2, Count(ptx, "max.f32"));
        Assert.Contains("DEFORM_GROUPED_BWD_INPUT_T:", ptx, StringComparison.Ordinal);
        Assert.Contains("DEFORM_GROUPED_BWD_INPUT_S:", ptx, StringComparison.Ordinal);
        Assert.Contains("DEFORM_GROUPED_BWD_INPUT_CO:", ptx, StringComparison.Ordinal);
        Assert.Equal(1, Count(ptx, "st.global.f32"));
        Assert.DoesNotContain(".shared", ptx, StringComparison.Ordinal);
        Assert.DoesNotContain("atom", ptx, StringComparison.Ordinal);
        Assert.Throws<NotSupportedException>(() =>
            PtxDeformableConv2DGroupedNchw3x3BackwardInputF32Kernel.EmitPtx(8, 9));
    }

    [Fact]
    public void DeformableConv2DGroupedBackwardInputManifestCell_IsExperimentalWithDedicatedRoute()
    {
        Assert.Equal(DirectPtxConvolutionCoverageStatus.ExperimentalDirectPtx,
            DirectPtxConvolutionCoverageManifest.Get("IEngine.DeformableConv2DGroupedBackward").Status);
        Assert.Equal(DirectPtxConvolutionCoverageStatus.ExperimentalDirectPtx,
            DirectPtxConvolutionCoverageManifest
                .Get("CudaBackend.TryDirectPtxDeformableConv2DGroupedBackwardInput").Status);
    }

    [SkippableFact]
    public void DriverOnlyDeformableConv2DGroupedBackwardInput_MatchesCpuReference()
    {
        Skip.IfNot(DirectPtxRuntime.IsAvailable, "Requires an NVIDIA CUDA driver and GPU.");
        using var runtime = new DirectPtxRuntime();
        Skip.IfNot(DirectPtxArchitecture.HasExperimentalConvolution(
        ...
        const int inChannels = PtxDeformableConv2DGroupedNchw3x3BackwardInputF32Kernel.InputChannels;
        const int outChannels = PtxDeformableConv2DGroupedNchw3x3BackwardInputF32Kernel.OutputChannels;
        const int height = PtxDeformableConv2DGroupedNchw3x3BackwardInputF32Kernel.Height;
        const int width = PtxDeformableConv2DGroupedNchw3x3BackwardInputF32Kernel.Width;
        const int kernelSize = PtxDeformableConv2DGroupedNchw3x3BackwardInputF32Kernel.KernelSize;
        const int taps = PtxDeformableConv2DGroupedNchw3x3BackwardInputF32Kernel.TapsPerChannel;
        const int channelsPerGroup = PtxDeformableConv2DGroupedNchw3x3BackwardInputF32Kernel.ChannelsPerGroup;
        const int offChannels = PtxDeformableConv2DGroupedNchw3x3BackwardInputF32Kernel.OffsetChannels;
        const int maskChannels = PtxDeformableConv2DGroupedNchw3x3BackwardInputF32Kernel.MaskChannels;
        int spatial = height * width;

        using var kernel = new PtxDeformableConv2DGroupedNchw3x3BackwardInputF32Kernel(runtime);
        using var gradOutDevice = runtime.AllocateBytes((nuint)PtxDeformableConv2DGroupedNchw3x3BackwardInputF32Kernel.GradOutputBytes);
        using var weightDevice = runtime.AllocateBytes((nuint)PtxDeformableConv2DGroupedNchw3x3BackwardInputF32Kernel.WeightBytes);
        using var offsetDevice = runtime.AllocateBytes((nuint)PtxDeformableConv2DGroupedNchw3x3BackwardInputF32Kernel.OffsetBytes);
        using var maskDevice = runtime.AllocateBytes((nuint)PtxDeformableConv2DGroupedNchw3x3BackwardInputF32Kernel.MaskBytes);
        using var gradInDevice = runtime.AllocateBytes((nuint)PtxDeformableConv2DGroupedNchw3x3BackwardInputF32Kernel.GradInputBytes);
        ...
        for (int ix = 0; ix < width; ix++)
        {
            ...
            for (int t = 0; t < taps; t++)
            {
                ...
                {
                    int s = oy * width + ox;
                    float py = oy + ky - 1 + offsets[(gg * 2 * taps + 2 * t) * spatial + s];
                    float px = ox + kx - 1 + offsets[(gg * 2 * taps + 2 * t + 1) * spatial + s];
                    float coeff = G(iy - py) * G(ix - px);
                    if (coeff == 0f) continue;
                    float gsum = 0;
                    for (int co = 0; co < outChannels; co++)
                        gsum += gradOut[co * spatial + s] * weights[((co * inChannels + ci) * kernelSize + ky) * kernelSize + kx];
                    acc += coeff * mask[(gg * taps + t) * spatial + s] * gsum;
                }
            }
            float got = actual[ci * spatial + (iy * width + ix)];
            maxAbsoluteError = MathF.Max(maxAbsoluteError, MathF.Abs(got - acc));
        ...
        Assert.True(maxAbsoluteError <= 3e-4f,
            $"Grouped deformable Conv2D backward-input max absolute error {maxAbsoluteError:G9}.");
    }
