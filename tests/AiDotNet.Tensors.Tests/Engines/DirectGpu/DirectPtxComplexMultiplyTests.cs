...
public class DirectPtxComplexMultiplyTests
{
    ...
    public void SpectralCoverageManifest_AssignsEveryCellExactlyOnce()
    {
        ...
        Assert.Equal(
            new[]
            {
                // Only a cell with a routed CudaBackend dispatch may be live. Every
                // other spectral kernel is checked in but unrouted, so it stays
                // PlannedDirectPtx until its dispatch and evidence land.
