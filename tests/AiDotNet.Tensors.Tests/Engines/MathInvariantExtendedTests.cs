...
public class MathInvariantExtendedTests
{
    ...
    // TensorMaskedFill (2)
    // ================================================================
    [Fact] public void MaskedFill_FillsTrue() { var x = R([4, 4], 1); var mask = new Tensor<bool>(Enumerable.Repeat(true, 16).ToArray(), [4, 4]); var r = E.TensorMaskedFill(x, mask, 99f).GetDataArray(); for (int i = 0; i < r.Length; i++) Assert.Equal(99f, r[i], Tol); }
    [Fact] public void MaskedFill_PreservesFalse() { var x = R([4, 4], 1); var mask = new Tensor<bool>(Enumerable.Repeat(false, 16).ToArray(), [4, 4]); AE(E.TensorMaskedFill(x, mask, 99f), x); }
    ...
    // TensorWhere (3)
    // ================================================================
    [Fact] public void Where_AllTrue_IsX() { var x = R([64], 1); var y = R([64], 2); var mask = new Tensor<bool>(Enumerable.Repeat(true, 64).ToArray(), [64]); AE(E.TensorWhere(mask, x, y), x); }
    [Fact] public void Where_AllFalse_IsY() { var x = R([64], 1); var y = R([64], 2); var mask = new Tensor<bool>(Enumerable.Repeat(false, 64).ToArray(), [64]); AE(E.TensorWhere(mask, x, y), y); }
    [Fact] public void Where_Shape() { var x = R([4, 8], 1); var y = R([4, 8], 2); var mask = new Tensor<bool>(Enumerable.Repeat(true, 32).ToArray(), [4, 8]); Assert.Equal(x.Shape, E.TensorWhere(mask, x, y).Shape); }
