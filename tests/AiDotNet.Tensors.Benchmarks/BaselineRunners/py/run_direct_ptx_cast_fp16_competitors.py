#!/usr/bin/env python3
"""Resident PyTorch/Triton competitors for issue #845 FP32->FP16 cast.

Measures the strongest resident NVIDIA paths for .to(float16) on identical device
tensors: eager, CUDA-graph, max-autotune compile, and compile+graph. The cast has
defined round-to-nearest-even semantics, so every path is bit-exact against the
reference. CPU MKL/OpenBLAS are intentionally ineligible.
"""
...
def print_header():
    print("PyTorch GPU competitors (same resident FP32->FP16 cast)")
    print(
...
def emit(run, size, method, device, e2e, managed_bytes, temporary_bytes, error):
    moved = size * 4.0 + size * 2.0
...
def main():
    ...
    for run in range(1, args.runs + 1):
        for size in SIZES:
            seed = 20268450 + run * 100 + (size % 9973)
            source = (((torch.arange(size, device="cuda", dtype=torch.int64) * 17 + seed)
                       % 4099 - 2049).float() / 32.0)
            torch.cuda.synchronize()
            expected = source.to(torch.float16).float()

            def eager():
                return source.to(torch.float16)

            probe = eager()
            error = (probe.float() - expected).abs().max().item()
            del probe
            eager_device = measure_device(eager)
            emit(run, size, "PyTorch CUDA eager", eager_device,
            ...
            try:
                ...
                torch.cuda.synchronize()
                graph_error = (probe.float() - expected).abs().max().item()
                emit(run, size, "PyTorch CUDA graph", eager_device,
                     measure_e2e(graph_operation), managed_peak_bytes(graph_operation),
                     graph_bytes, graph_error)
            except Exception as exception:
                print(f"SKIP run={run} size={size} PyTorch CUDA graph: "
            ...
            try:
                ...
                probe = compiled()
                torch.cuda.synchronize()
                compiled_error = (probe.float() - expected).abs().max().item()
                del probe
                compiled_device = measure_device(compiled)
                emit(run, size, "PyTorch compile", compiled_device,
                ...
                torch.cuda.synchronize()
                compiled_graph_error = (probe.float() - expected).abs().max().item()
                emit(run, size, "PyTorch compile graph", compiled_device,
