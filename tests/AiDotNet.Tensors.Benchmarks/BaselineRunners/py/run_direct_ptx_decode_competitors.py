#!/usr/bin/env python3
"""Forced PyTorch CUDA baselines for issue #834 FP32 decode attention.

The dense ABI matches the direct-PTX decode specialization exactly: B=1,
Sq=1, D=64, FP32 BHSD input, explicit scale=1/8, and native MHA/GQA/MQA
head mapping. Paged KV is intentionally absent because PyTorch's public SDPA
API has no equivalent block-table ABI; that domain is compared to AiDotNet's
existing paged CUDA kernel in the C# championship harness.
"""
...
def emit(record, json_lines):
    if json_lines:
        ...
        return
    print(
        f"{record['run']:3d} {record['shape']:<11} {record['method']:<27} "
...
def main():
    ...
    if not args.json_lines:
        print(
            ...
            f"CUDA={torch.version.cuda}; FP32; 30 warmups + 101 synchronized E2E samples"
        )
        print(
            f"{'Run':>3} {'Shape':<11} {'Method':<27} {'median us':>10} "
    ...
    for run in range(1, args.runs + 1):
        torch.manual_seed(20260900 + run)
        for shape_name, hq, hkv, sequence in SHAPES:
            q = (torch.rand((1, hq, 1, D), device=device, dtype=torch.float32) - 0.5) * 0.5
            k = (torch.rand((1, hkv, sequence, D), device=device, dtype=torch.float32) - 0.5) * 0.5
            v = (torch.rand((1, hkv, sequence, D), device=device, dtype=torch.float32) - 0.5) * 0.5

            def operation():
                return F.scaled_dot_product_attention(
                    q, k, v, dropout_p=0.0, is_causal=False,
                    scale=0.125, enable_gqa=hq != hkv
                )
            ...
            for backend_name, backend in backends:
                try:
                    # Backend selection is resident configuration and remains
                    # outside each timed invocation. Timing includes public
                    # dispatch, output creation, launch, and synchronization.
