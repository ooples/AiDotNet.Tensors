#!/usr/bin/env python3
"""Resident PyTorch/Triton competitors for issue #843 FP32 row-sum reduction.

Measures the strongest resident NVIDIA paths on identical device tensors:
eager, CUDA-graph, max-autotune compile, and compile+graph. Error is scored
against a double-precision oracle so the comparison is apples-to-apples with the
direct-PTX kernel (whose only disclosed asymmetry is fp32 tree-order rounding).
CPU MKL/OpenBLAS are intentionally ineligible for this GPU comparison.
"""
...
def print_header():
    print("PyTorch GPU competitors (same resident contiguous FP32 row-sum)")
    print(
...
def emit(run, rows, columns, method, device, e2e, managed_bytes, temporary_bytes, error):
    ...
    bandwidth = (elements * 4.0 + rows * 4.0) / (device["median"] * 1.0e-6) / 1.0e9
    print(
...
def main():
    ...
    for run in range(1, args.runs + 1):
        for rows, columns in SHAPES:
            seed = 20268430 + run * 100 + rows + columns
            ...
            del indices
            torch.cuda.synchronize()
            expected = input_tensor.double().sum(dim=-1)
