#!/usr/bin/env python3
"""Resident PyTorch/Triton competitors for issue #844 FP32 embedding gather.

Measures the strongest resident NVIDIA paths for index_select on identical
device tensors: eager, CUDA-graph, max-autotune compile, and compile+graph.
Gather is a pure copy, so every path is bit-exact against the reference; the
comparison is unambiguous. CPU MKL/OpenBLAS are intentionally ineligible.
"""
...
def print_header():
    print("PyTorch GPU competitors (same resident FP32 embedding gather / index_select)")
    print(
        "run nidx  feat  method                    dev mean/med/p95/p99 us       "
        "e2e mean/med/p95/p99 us       GB/s allocB tmpB err"
    )
    print("-" * 168)


def emit(run, nidx, feat, method, device, e2e, managed_bytes, temporary_bytes, error):
    moved = nidx * feat * 4.0 * 2.0 + nidx * 4.0
    bandwidth = moved / (device["median"] * 1.0e-6) / 1.0e9
    print(
        f"{run:3d} {nidx:5d} {feat:5d} {method:<25} "
...
def main():
    ...
    for run in range(1, args.runs + 1):
        for nidx, feat in SHAPES:
            seed = 20268440 + run * 100 + nidx + feat
            table = ((torch.arange(TABLE_ROWS * feat, device="cuda", dtype=torch.int64)
                      * 17 + seed) % 251 - 125).float().reshape(TABLE_ROWS, feat) / 16.0
            indices = ((torch.arange(nidx, device="cuda", dtype=torch.int64) * 131 + seed)
                       % TABLE_ROWS)
            torch.cuda.synchronize()
            expected = table.index_select(0, indices).double()
            ...
            emit(run, nidx, feat, "PyTorch CUDA eager", eager_device,
            ...
            try:
                ...
                emit(run, nidx, feat, "PyTorch CUDA graph", eager_device,
                     measure_e2e(graph_operation), managed_peak_bytes(graph_operation),
                     graph_bytes, graph_error)
            except Exception as exception:
                print(f"SKIP run={run} nidx={nidx} feat={feat} PyTorch CUDA graph: "
            ...
            try:
                ...
                emit(run, nidx, feat, "PyTorch compile", compiled_device,
                ...
                emit(run, nidx, feat, "PyTorch compile graph", compiled_device,
                     measure_e2e(compiled_graph), managed_peak_bytes(compiled_graph),
                     compiled_graph_bytes, compiled_graph_error)
            except Exception as exception:
                print(f"SKIP run={run} nidx={nidx} feat={feat} PyTorch compile: "
                      f"{type(exception).__name__}: {exception}")

            del table, indices, expected
