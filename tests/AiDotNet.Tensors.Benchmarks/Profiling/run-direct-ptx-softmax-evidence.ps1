<#
.SYNOPSIS
    Direct-PTX row-softmax (#840) release-evidence runner.

.DESCRIPTION
    Runs the softmax championship experiment in N independent, freshly launched
    OS processes (default 3, the release-gate minimum), aggregates the
    machine-readable "softmax_evidence_json=" lines each process emits, and
    evaluates the DirectPtxReleaseGate thresholds per shape:
        ...
        - max absolute error <= tolerance (default 5e-5)
        - at least 3 independent runs

    Exit code 0 = every promoted shape cleared the gate. Non-zero = at least one
    shape is on HOLD (printed with the failing reasons), or the run was
    incomplete. Writes an aggregated manifest.json to the output directory.

    You run this on the measured GPU (GA10x/SM86); it does not run in CI.

.EXAMPLE
    pwsh tests/AiDotNet.Tensors.Benchmarks/Profiling/run-direct-ptx-softmax-evidence.ps1 -Runs 3
#>
[CmdletBinding()]
param(
    [ValidateRange(3, 20)]
    [int]$Runs = 3,
    [string]$OutputDirectory = (Join-Path ([System.IO.Path]::GetTempPath()) ("aidotnet-ptx-softmax-evidence-" + (Get-Date -Format 'yyyyMMdd-HHmmss'))),
...
$flag = '--direct-ptx-softmax'
$evidencePrefix = 'softmax_evidence_json='
...
foreach ($shapeGroup in $byShape) {
    ...
    foreach ($m in $byMethod) {
        ...
        $methodAgg[$m.Name] = [pscustomobject]@{
            ...
            MaxError     = [double](@($m.Group | ForEach-Object { [double]$_.max_error } | Measure-Object -Maximum).Maximum)
