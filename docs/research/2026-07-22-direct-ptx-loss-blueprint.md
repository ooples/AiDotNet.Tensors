...
contiguous SM86 FP32 candidate: a warp-per-row **per-sample mean-squared-error
...
assigns the MAE, Huber, BCE, BCE-with-logits, cross-entropy, NLL,
...
(2048,128), (8192,128)`; exact GA102/SM86 only; else exact-reason fallback.
...
The resident Python PyTorch competitor script measures four variants — eager,
...
That script is the authoritative four-variant PyTorch comparison. The in-process
C# harness intentionally measures PyTorch eager only because TorchSharp does not
expose `torch.compile` or CUDA-graph capture; its diagnostic release-gate output
therefore chooses the fastest result available from AiDotNet `mse_loss` and
PyTorch eager, and must not be treated as the complete promotion comparison.
...
with exactly three experimental cells (`CudaBackend.MaeBackward`,
`CudaBackend.MseLoss`, and `CudaBackend.MseLossBackward`) and no promoted cell,
and (on a validated SM86 device) mean-MSE parity within relative tolerance with
zero local and static-shared bytes and at least three active blocks per SM.
The backward emitters and their closed shape/scale domains have separate
structural coverage in `DirectPtxLossBackwardTests.cs`.
...
4. Add FP16/BF16 families and independently measured Ada, Hopper, and
   Blackwell. The landed MSE backward path computes
   `grad = 2/N (pred-target) gradOut`; the established MAE gradient intentionally
   applies neither `1/N` scaling nor `gradOut`.
