...
| 2 | Coalesced vector memory access | Thread-per-element kernels (complex, octonion, RBF, pairwise, ANN cells, hash-encode) use adjacent per-lane `.f32`/`.u8` loads and a single final store; block-per-row reductions stride the row by `blockDim` for coalesced partials. **Pass (mechanically); vectorized `v2/v4` widening is a follow-up.** |
