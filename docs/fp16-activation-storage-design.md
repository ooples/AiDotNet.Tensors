...
- **P2 — REMAINING (compiled lazy-graph port + GPU).** Port the proven eager-tape mechanism into the compiled training path so FP16-eligible ops (matmul/FF) emit `Tensor<Half>` activation buffers (`Hgemm` output) — *the actual resident-memory win*. Concretely: (a) add a `BackwardFunction` to `CrossTypeLazyNode` (today forward-only, FFT/Complex), the lazy-graph analog of the cast-with-backward already verified on the tape; (b) thread a **secondary Half grads space** through `CompiledTrainingPlan`'s backward replay / `CompiledBackwardWalk`, with the cast nodes as the bridge (same Gauss-Seidel structure, now over compiled steps); (c) the autocast policy emits `Half` activation buffers at trace time for eligible ops. Gated by the same gradient-check (CPU) **and** at-scale GPU validation. This is hot-path surgery on the just-stabilized compiled backward, so it lands as its own validated pass, behind `AIDOTNET_AUTOCAST=fp16`, default FP32 byte-identical.
- **P4 — REMAINING (validation).** d512/L6/B256: resident ≈ ½ (≤ ~12GB) via `opt-parity LEAKPROBE`, loss-parity to AMP tolerance, 0 NaN end-to-end; extend the gradient-check CI to the compiled mixed-precision node.

**Status:** the hard, novel core — a correct, loss-scaled, mixed-dtype reverse-mode autograd engine with the default FP32 path byte-identical — is **landed and verified** on branch `feat/fp16-activation-storage` (commits: cast bridge → tape integration → loss scaling). What remains (P2/P4) is integrating that proven engine into the compiled GPU plan and validating the memory number at scale.
