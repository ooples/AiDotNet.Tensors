...
- **P2 — backward machinery DONE (verified, CPU); forward-emission + production wiring REMAINING (GPU).**
  ...
  - **REMAINING (c) — forward Half-buffer emission + production wiring (GPU-bound).** Under `AIDOTNET_AUTOCAST=fp16`, graph-mode tracing must emit, for eligible ops (matmul/FF), a down-cast (`CrossTypeLazyNode`) → `Hgemm` producing a **`Tensor<Half>` buffer** → up-cast at the next FP32 op — so the activation buffer is genuinely 2 bytes (the resident-memory win). Then `CompiledTrainingPlan`'s backward replay calls the `MixedPrecisionGraphBackward` algorithm (secondary Half grad map). `Hgemm` is CUDA-only and the win is GPU buffers, so this step is validated on GPU. Behind the env flag; default FP32 byte-identical.
- **P4 — REMAINING (validation, GPU).** d512/L6/B256: resident ≈ ½ (≤ ~12GB) via `opt-parity LEAKPROBE`, loss-parity to AMP tolerance, 0 NaN end-to-end.

**Status:** the entire mixed-dtype **autograd backward machinery** — cast bridge, seeded cross-tape backward, loss scaling, the cross-type lazy-node backward, and the single-pass unified-graph backward — is **landed and CPU-verified at both the eager-tape and lazy-graph levels**, default FP32 path byte-identical, on branch `feat/fp16-activation-storage` (commits: cast bridge → tape integration → loss scaling → CrossTypeLazyNode backward → unified-graph backward). What remains is the **forward** side — emitting `Half` activation buffers under autocast and wiring the graph-backward algorithm into the production compiled plan — plus the at-scale GPU validation of the ½-resident number. Both are GPU-bound (`Hgemm` is CUDA-only; the memory win is GPU buffers).
