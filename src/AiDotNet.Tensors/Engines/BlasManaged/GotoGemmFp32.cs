...
internal static class GotoGemmFp32
{
    ...
    /// <summary>SHORT-M cooperative shared-A path (#88, the diffusion lever). For short M the A matrix is
    /// small and L3-resident (e.g. DiT QKV A=256×1152×4≈1.1MB ≪ a CCX's 16MB L3), yet <see cref="RunParallel"/>
    /// re-packs each ic-block's A-panel once PER jc-block (≈n/nc× for wide-N) — PerfView blamed that redundant
    /// pack for 86% of LLC misses. This packs the WHOLE A exactly ONCE into one shared buffer, then parallelizes
    /// the IC×JC grid with every worker READING the shared packed-A and packing only its own (disjoint, non-
    /// redundant) B-panel. Bit-exact with RunParallel (same kernel, same fixed K-order, disjoint C writes).
    /// This is the pack-A-once extreme: it trades the per-thread A-pack work for cross-CCX L3 reads of the one
    /// shared copy — measure before routing (the per-CCX A-REPLICATE variant was MIXED; this is the opposite
    /// extreme). Serial A-pack (A is small for short-M; the wide-N compute dwarfs it).
    ///
    /// DEAD END (2026-06-28, <c>--ab-shared-a</c>, bit-exact vs per-tile): MEASURED 0.64-0.85× of
    /// <see cref="RunParallel"/> on every short-M shape (dit-qkv 0.83×, attnout 0.68×, mlp1 0.85×, mlp2 0.64×,
    /// 384×1024 0.71×, 512×2048 0.75×). The pack-A-once extreme LOSES: the cross-CCX L3 reads of the single
    /// shared A copy cost more than the per-thread A-pack they remove — i.e. the "redundant" per-tile pack buys
    /// CCX/L2-local A reads that are worth more than the pack work. Conclusion (with per-tile=best,
    /// per-CCX-replicate=mixed): A-pack TRAFFIC is not the lever; A-read LOCALITY is, and per-tile is already
    /// near-optimal for it. NOT routed — retained behind the bench as the logged dead-end so it isn't rebuilt.</summary>
    internal static unsafe void RunParallelSharedA(
        float* a, int lda, float* b, int ldb, float* c, int ldc,
        int m, int n, int k, int mc, int nc, int kc)
    {
        if (k <= 0) { ZeroCBlock(c, ldc, 0, 0, m, n); return; }
        int numIc = (m + mc - 1) / mc, numJc = (n + nc - 1) / nc;
        int totalTiles = numIc * numJc;
        if (totalTiles <= 1) { RunSingle(a, lda, b, ldb, c, ldc, m, n, k, mc, nc, kc); return; }

        // Per-ic-block whole-K packed-A offsets into one shared buffer (last block may be partial).
        long[] offs = new long[numIc + 1];
        for (int ib = 0; ib < numIc; ib++)
        {
            int effMc = Math.Min(mc, m - ib * mc);
            offs[ib + 1] = offs[ib] + PackedALen(effMc, k, kc);
        }
        float[] sharedAArr = ArrayPool<float>.Shared.Rent(checked((int)offs[numIc]) + 8); // +8: PackA store overrun pad
        try
        {
            fixed (float* sharedA = sharedAArr)
            {
                // Pack the whole A once (serial — A is small for short-M; PackAPanel is the same layout
                // RunBlockPackedAB/RunTilePackedA read: K-panel p at p·mTiles·kc·Mr, Mr-tile mt at +mt·kc·Mr).
                for (int ib = 0; ib < numIc; ib++)
                {
                    int ic = ib * mc; int effMc = Math.Min(mc, m - ic);
                    PackAPanel(a, lda, ic, effMc, k, kc, sharedA + offs[ib]);
                }
                nint ai = (nint)a, bi = (nint)b, ci = (nint)c, sa = (nint)sharedA;
                int numIcL = numIc, mcL = mc, ncL = nc, kcL = kc, mL = m, nL = n, kL = k, ldaL = lda, ldbL = ldb, ldcL = ldc;
                long[] offsL = offs;
                CpuParallelSettings.ParallelForOrSerial(0, totalTiles, (long)m * n * k, tileIdx =>
                {
                    int ib = (int)(tileIdx % numIcL);
                    int ic = ib * mcL;
                    int jc = (int)(tileIdx / numIcL) * ncL;
                    int effMc = Math.Min(mcL, mL - ic);
                    int effNc = Math.Min(ncL, nL - jc);
                    if (effMc <= 0 || effNc <= 0) return;
                    RunTilePackedA((float*)ai, ldaL, (float*)bi, ldbL, (float*)ci, ldcL, ic, jc, effMc, effNc,
                        kL, mcL, ncL, kcL, (float*)sa + offsL[ib]);
                }, deterministicSafe: true);
            }
        }
        finally { ArrayPool<float>.Shared.Return(sharedAArr); }
    }

    /// <summary>Like <see cref="RunTilePackedB"/> but the ROLES are swapped: A is already whole-K packed (by
    /// <see cref="PackAPanel"/>) in <paramref name="packedA"/> and SHARED across workers; this tile packs only
    /// its own per-kc B-panel and runs the kernel + tails (tails read original a,b). Deterministic (fixed K
    /// order, disjoint C). Used by <see cref="RunParallelSharedA"/> for the short-M diffusion shapes.</summary>
    internal static unsafe void RunTilePackedA(
        float* a, int lda, float* b, int ldb, float* c, int ldc,
        int ic, int jc, int effMc, int effNc, int k, int mc, int nc, int kc, float* packedA)
    {
                ...
                for (int p = 0, pc = 0; pc < k; p++, pc += kc)
                {
                    ...
                    for (int nt = 0; nt < nTiles; nt++)
                    {
                        ...
                        for (int kk = 0; kk < effKc; kk++)
                        {
                            ...
                            Vector256.Store(Vector256.Load(brow), d);          // SIMD copy of the Nr=16 contiguous B-cols
        ...
        finally { ArrayPool<float>.Shared.Return(pbArr); }
    }
