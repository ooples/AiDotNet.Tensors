            ...
            && gte.GetBackend() is Engines.DirectGpu.CUDA.CudaBackend cb)
        {
            ...
            if (_graphStepCalls > GraphWarmupSteps)
            {
                ...
                if (_stepGraphExec == IntPtr.Zero)
                {
                    ...
                    try
                    {
                        // CAPTURE-PATH SCOPE (issue #26): the resident-buffer op branches engage ONLY inside this
                    ...
                    catch
                    {
                        ...
                        Helpers.DeferredArrayMaterializer.FlushAllAnyThread();   // ALL threads' entries — pool-thread registrations included
                    ...
                    if (exec == IntPtr.Zero)
                    {
                        ...
                        _graphStepDisabled = true;
                        Helpers.DeferredArrayMaterializer.FlushAllAnyThread();   // flush ALL pending downloads (any thread) before buffers are freed
                        gte.ResumeActivationEviction();
                        _graphEvictionSuspended = false;
                        return StepEager();
                    }
