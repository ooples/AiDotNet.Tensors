...
public sealed partial class VulkanBackend
{
    ...
    // Backward gated activations — CPU fallback since proper backward GLSL kernels
    // need to compute both activation derivative and gate gradient separately.
    public void GluBackward(IGpuBuffer go, IGpuBuffer inp, IGpuBuffer gi, int os, int hd) { float[] goD = DownloadBuffer(go), inD = DownloadBuffer(inp); int fd = hd * 2; float[] giD = new float[os * fd]; for (int j = 0; j < os; j++) for (int k = 0; k < hd; k++) { float v = inD[j * fd + k], g = inD[j * fd + hd + k]; float sig = 1f / (1f + MathF.Exp(-g)); giD[j * fd + k] = goD[j * hd + k] * sig; giD[j * fd + hd + k] = goD[j * hd + k] * v * sig * (1f - sig); } UploadToBuffer(giD, gi); }
    ...
    public void GeGluBackward(IGpuBuffer go, IGpuBuffer inp, IGpuBuffer gi, int os, int hd) { float[] goD = DownloadBuffer(go), inD = DownloadBuffer(inp); int fd = hd * 2; float[] giD = new float[os * fd]; for (int j = 0; j < os; j++) for (int k = 0; k < hd; k++) { float v = inD[j * fd + k], g = inD[j * fd + hd + k]; float x3 = v * v * v; float gelu = 0.5f * v * (1f + MathF.Tanh(0.7978845608f * (v + 0.044715f * x3))); giD[j * fd + k] = goD[j * hd + k] * g; giD[j * fd + hd + k] = goD[j * hd + k] * gelu; } UploadToBuffer(giD, gi); }
    ...
    public void ReGluBackward(IGpuBuffer go, IGpuBuffer inp, IGpuBuffer gi, int os, int hd) { float[] goD = DownloadBuffer(go), inD = DownloadBuffer(inp); int fd = hd * 2; float[] giD = new float[os * fd]; for (int j = 0; j < os; j++) for (int k = 0; k < hd; k++) { float v = inD[j * fd + k], g = inD[j * fd + hd + k]; giD[j * fd + k] = goD[j * hd + k] * g * (v > 0 ? 1f : 0f); giD[j * fd + hd + k] = goD[j * hd + k] * Math.Max(v, 0f); } UploadToBuffer(giD, gi); }
    ...
    public void SwiGluBackward(IGpuBuffer go, IGpuBuffer inp, IGpuBuffer gi, int os, int hd) { float[] goD = DownloadBuffer(go), inD = DownloadBuffer(inp); int fd = hd * 2; float[] giD = new float[os * fd]; for (int j = 0; j < os; j++) for (int k = 0; k < hd; k++) { float v = inD[j * fd + k], g = inD[j * fd + hd + k]; float sig = 1f / (1f + MathF.Exp(-v)); float swish = v * sig; giD[j * fd + k] = goD[j * hd + k] * g * (sig + v * sig * (1f - sig)); giD[j * fd + hd + k] = goD[j * hd + k] * swish; } UploadToBuffer(giD, gi); }
