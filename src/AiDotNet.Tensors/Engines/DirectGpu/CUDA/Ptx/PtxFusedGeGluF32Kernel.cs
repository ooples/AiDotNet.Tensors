...
/// <summary>
/// Exact contiguous FP32 tanh-GeGLU specialization over [value | gate] rows.
/// Four output elements remain lane-private from their two vector loads,
/// through GELU(gate), to the sole vector store.
/// </summary>
internal sealed class PtxFusedGeGluF32Kernel : IDisposable
{
    internal const string EntryPoint = "aidotnet_fused_geglu_f32x4";
    ...
    internal PtxFusedGeGluF32Kernel(
        ...
        int halfDimension)
    {
        ...
        ptx.AppendLine("    @%p0 bra GEGLU_RETURN;");
        ...
        for (int lane = 0; lane < 4; lane++)
        {
            ...
            int result = 12 + lane;
            ptx.AppendLine($"    mul.rn.f32 %f{temporary}, %f{gate}, %f{gate};");
            ptx.AppendLine($"    mul.rn.f32 %f{temporary}, %f{temporary}, %f{gate};");
            ptx.AppendLine($"    fma.rn.f32 %f{temporary}, %f{temporary}, 0f3D372713, %f{gate};");
            ptx.AppendLine($"    mul.rn.f32 %f{temporary}, %f{temporary}, 0f3F4C422A;");
            ptx.AppendLine($"    tanh.approx.f32 %f{temporary}, %f{temporary};");
            ptx.AppendLine($"    add.rn.f32 %f{temporary}, %f{temporary}, 0f3F800000;");
            ptx.AppendLine($"    mul.rn.f32 %f{temporary}, %f{temporary}, %f{gate};");
            ptx.AppendLine($"    mul.rn.f32 %f{temporary}, %f{temporary}, 0f3F000000;");
            ptx.AppendLine($"    mul.rn.f32 %f{result}, %f{lane}, %f{temporary};");
        }
        ptx.AppendLine("    st.global.v4.f32 [%rd7], {%f12,%f13,%f14,%f15};");
        ptx.AppendLine("GEGLU_RETURN:");
        ptx.AppendLine("    ret;");
        ptx.AppendLine("}");
        return ptx.ToString();
    }
        ...
        int halfDimension)
    {
        ...
        return new DirectPtxKernelBlueprint(
            ...
            {
                ["formula"] = "value*gelu_tanh(gate)",
                ["split"] = "input[row,0:D]=value; input[row,D:2D]=gate",
                ["mode"] = "inference-forward-tanh-approximation",
                ["input"] = "fp32",
    ...
    private static void Validate(int outerSize, int halfDimension)
    {
        if (!IsSupportedShape(outerSize, halfDimension))
            throw new ArgumentOutOfRangeException(nameof(outerSize),
                "The first GeGLU family supports exact (outer,D) buckets " +
