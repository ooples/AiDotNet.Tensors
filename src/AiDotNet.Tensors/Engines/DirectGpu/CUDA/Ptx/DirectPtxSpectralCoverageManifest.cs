...
internal static class DirectPtxSpectralCoverageManifest
{
    ...
    [
        ...
        Experimental("CudaBackend.WidebandLogBinPool", "NVRTC wideband_log_bin_pool", "logarithmic magnitude bin pooling", Frames, "FP32", "v1 Ampere thread-per-(seg,bin) (PtxWidebandLogBinPoolF32Kernel); quadratic log-bin edges, mean over range, lg2.approx(1+avg)*ln(2), tolerance-based"),
        ...
        Experimental("CudaBackend.CavityBounceInplace", "NVRTC cavity_bounce_inplace", "post-IFFT cavity nonlinearity", Split, "FP32", "v1 Ampere elementwise (PtxCavityBounceInplaceF32Kernel); tanh.approx of clamp(real*invN,-20,20), imag zeroed, invN .param .f32, tolerance-based"),
        ...
        Experimental("CudaBackend.Atan2Elementwise", "NVRTC atan2_elementwise", "elementwise complex phase", Split, "FP32", "same split-buffer atan2(imag,real) as SplitComplexPhase (PtxSplitComplexPhaseF32Kernel); minimax atan2 over the exact counts, tolerance-based"),
        ...
        Experimental("CudaBackend.NormalizeRowsFused", "NVRTC normalize_rows_fused", "fused per-row L2 normalization", "canonical contiguous [row, feature]", "FP32", "v1 Ampere one-block-per-row (PtxNormalizeRowsFusedF32Kernel); fma sum-of-squares, 256-wide shared-memory tree reduction under bar.sync, rsqrt.approx scale, tolerance-based"),
