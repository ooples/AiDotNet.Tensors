...
/// <summary>
/// Hand-emitted mixed-precision NCHW 1x1 convolution: FP16 input and weights, FP32
/// accumulation and output. <c>out[k,y,x] = sum_c W_half[k,c] * in_half[c,y,x]</c>
/// over the exact N1/C64/H16/W16/K64 geometry. Each thread owns one output element,
/// widens the FP16 operands with <c>cvt.f32.f16</c>, and accumulates the C-length
/// dot in a single FP32 register. Zero global intermediates, zero local bytes.
/// </summary>
internal sealed class PtxConv2DFp16K1NchwF32Kernel : IDisposable
{
    internal const string EntryPoint = "aidotnet_conv2d_fp16_k1_n1_c64_h16_w16_k64";
    ...
    internal const long InputBytes = (long)Batch * InputChannels * SpatialElements * 2;   // FP16
    internal const long WeightBytes = (long)OutputChannels * InputChannels * 2;            // FP16
    internal const long OutputBytes = (long)OutputElements * sizeof(float);                // FP32
    ...
    internal PtxConv2DFp16K1NchwF32Kernel(DirectPtxRuntime runtime)
    {
        PtxCompat.ThrowIfNull(runtime, nameof(runtime));
        if (!DirectPtxArchitecture.HasExperimentalConvolution(
                runtime.ComputeCapabilityMajor, runtime.ComputeCapabilityMinor))
            throw new NotSupportedException(
                $"FP16 1x1 conv has no experimental SM {runtime.ComputeCapabilityMajor}.{runtime.ComputeCapabilityMinor} specialization.");
        ...
        DirectPtxArchitectureFamily architecture)
    {
        ...
        return new DirectPtxKernelBlueprint(
            Operation: "conv2d-fp16-k1",
            Version: 1,
            Architecture: architecture,
            Variant: "n1-c64-h16-w16-k64-r1-s1-p0-fp16in-fp32out",
            Tensors:
            [
                ...
                new("weights", DirectPtxPhysicalType.Float16, DirectPtxPhysicalLayout.Oihw,
            ...
            {
                ["equation"] = "out_fp32[k,y,x] = sum_c W_half[k,c] * in_half[c,y,x]",
                ["input"] = "fp16",
                ["weights"] = "fp16",
                ["accumulator"] = "fp32-fma (widened via cvt.f32.f16)",
                ["output"] = "fp32",
                ["layout"] = "nchw/oihw",
    ...
    internal static string EmitPtx(int computeCapabilityMajor, int computeCapabilityMinor)
    {
        if (!DirectPtxArchitecture.HasExperimentalConvolution(
            ...
            throw new NotSupportedException(
                "Only the experimental SM86 FP16 1x1 conv emitter exists.");

        const int inChannelStrideBytes = SpatialElements * 2;   // FP16 per input channel
        const int weightChannelStrideBytes = 2;                 // FP16 per input channel of a weight row
        ...
        ptx.AppendLine("    shr.u32 %r4, %r2, 8;");                            // output channel k
        // Weight row base for k: weights_ptr + k*(C*2).
        ptx.AppendLine($"    mul.wide.u32 %rd4, %r4, {InputChannels * 2};");
        ptx.AppendLine("    add.u64 %rd4, %rd1, %rd4;");
        // Input base for this spatial position: in_ptr + spatial*2.
        ptx.AppendLine("    mul.wide.u32 %rd5, %r3, 2;");
        ptx.AppendLine("    add.u64 %rd5, %rd0, %rd5;");
        ptx.AppendLine("    mov.f32 %f0, 0f00000000;");
        for (int channel = 0; channel < InputChannels; channel++)
        {
            // in_half[c, spatial]: base + c*(H*W*2).
            if (channel == 0)
                ptx.AppendLine("    ld.global.nc.b16 %h0, [%rd5];");
            else
                ptx.AppendLine($"    ld.global.nc.b16 %h0, [%rd5+{channel * inChannelStrideBytes}];");
            ptx.AppendLine("    cvt.f32.f16 %f1, %h0;");
            // W_half[k, c]: weight row base + c*2.
            if (channel == 0)
                ptx.AppendLine("    ld.global.nc.b16 %h1, [%rd4];");
            else
                ptx.AppendLine($"    ld.global.nc.b16 %h1, [%rd4+{channel * weightChannelStrideBytes}];");
            ptx.AppendLine("    cvt.f32.f16 %f2, %h1;");
