...
/// <summary>
/// Hand-emitted FP32 NCHW general multi-channel 3x3 convolution with fused bias and
/// ReLU (stride 1, zero pad 1):
/// <c>out[co,y,x] = relu(bias[co] + sum_ci sum_ky sum_kx W[co,ci,ky,kx] * in[ci, y+ky-1, x+kx-1])</c>
/// over the exact N1/Cin64/H16/W16/Cout64 geometry. The bias add and activation are
/// folded into the accumulator's final register, so no intermediate convolution
/// result is written to global memory. Structurally identical to
/// <see cref="PtxConv2DNchw3x3ForwardF32Kernel"/> plus a four-instruction epilogue.
/// </summary>
internal sealed class PtxFusedConv2DNchw3x3BiasReluF32Kernel : IDisposable
{
    internal const string EntryPoint = "aidotnet_conv2d_n1_cin64_h16_w16_cout64_r3_s1_p1_bias_relu";
    ...
    internal PtxFusedConv2DNchw3x3BiasReluF32Kernel(DirectPtxRuntime runtime)
    {
        PtxCompat.ThrowIfNull(runtime, nameof(runtime));
        if (!DirectPtxArchitecture.HasExperimentalConvolution(
                runtime.ComputeCapabilityMajor, runtime.ComputeCapabilityMinor))
            throw new NotSupportedException(
                $"Fused Conv2D 3x3 has no experimental SM {runtime.ComputeCapabilityMajor}.{runtime.ComputeCapabilityMinor} specialization.");
        ...
        DirectPtxArchitectureFamily architecture)
    {
        ...
        return new DirectPtxKernelBlueprint(
            ...
            {
                ["equation"] = "relu(bias[co] + sum_ci sum_ky sum_kx W[co,ci,ky,kx] * in[ci, y+ky-1, x+kx-1])",
                ["input"] = "fp32",
    ...
    internal static string EmitPtx(int computeCapabilityMajor, int computeCapabilityMinor)
    {
        if (!DirectPtxArchitecture.HasExperimentalConvolution(
            ...
            throw new NotSupportedException(
                "Only the experimental SM86 fused Conv2D 3x3 emitter exists.");
        ...
        ptx.AppendLine($"    mul.wide.u32 %rd4, %r6, {weightOutStrideBytes};");
        ptx.AppendLine("    add.u64 %rd4, %rd1, %rd4;");                        // weight base for co (advance +36)
        ptx.AppendLine("    mul.wide.u32 %rd5, %r3, 4;");
        ptx.AppendLine("    add.u64 %rd5, %rd0, %rd5;");                        // input center base (advance +1024)
        ptx.AppendLine("    mov.f32 %f0, 0f00000000;");
        ptx.AppendLine("    mov.u32 %r7, 0;");                                  // input-channel counter
        ptx.AppendLine("CONV2D_FUSED_CHANNELS:");
        ...
        ptx.AppendLine("    @%p5 bra CONV2D_FUSED_CHANNELS;");
