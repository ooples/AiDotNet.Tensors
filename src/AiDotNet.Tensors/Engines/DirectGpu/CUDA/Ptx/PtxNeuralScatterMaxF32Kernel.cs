...
/// <summary>
/// Exact deterministic replacement for CudaNeuralNetKernels.scatter_max.
/// One thread owns each destination cell, scans source rows in fixed order,
/// and writes the public Int32 argmax ABI without an intermediate buffer.
/// </summary>
internal sealed class PtxNeuralScatterMaxF32Kernel : IDisposable
{
    internal const string EntryPoint =
        "aidotnet_neural_scatter_max_f32_src16384_dst1024_f64";
    ...
    internal PtxNeuralScatterMaxF32Kernel(DirectPtxRuntime runtime)
    {
        ArgumentNullException.ThrowIfNull(runtime);
        if (!DirectPtxArchitecture.HasValidatedSparseGraph(
            runtime.ComputeCapabilityMajor, runtime.ComputeCapabilityMinor))
            throw new NotSupportedException(
                $"Neural scatter-max has no SM {runtime.ComputeCapabilityMajor}.{runtime.ComputeCapabilityMinor} experimental specialization.");
        ...
        DirectPtxTensorView argmax)
    {
        ...
        if (Overlaps(output, source) || Overlaps(output, indices) ||
            Overlaps(output, argmax) || Overlaps(argmax, source) || Overlaps(argmax, indices))
            throw new ArgumentException(
                "Neural scatter-max outputs must be mutually disjoint and disjoint from inputs.");
        IntPtr p0 = source.Pointer;
        IntPtr p1 = indices.Pointer;
        IntPtr p2 = output.Pointer;
        IntPtr p3 = argmax.Pointer;
        ...
        DirectPtxArchitectureFamily architecture)
    {
        ...
        return new DirectPtxKernelBlueprint(
            Operation: "neural-scatter-max-f32",
            Version: 1,
            ...
            [
                ...
                Exact("output", DirectPtxPhysicalType.Float32,
                    DirectPtxPhysicalLayout.RowMajor2D, output, DirectPtxTensorAccess.Write),
                Exact("argmax", DirectPtxPhysicalType.Int32,
                    DirectPtxPhysicalLayout.RowMajor2D, output, DirectPtxTensorAccess.Write)
            ],
    ...
    internal static string EmitPtx(int ccMajor, int ccMinor)
    {
        ...
        ptx.AppendLine("NEURAL_SCATTER_MAX_LOOP:");
        ptx.AppendLine($"    setp.ge.u32 %p0, %r5, {SourceRows};");
        ptx.AppendLine("    @%p0 bra NEURAL_SCATTER_MAX_DONE;");
        ptx.AppendLine("    mul.wide.u32 %rd4, %r5, 4;");
        ptx.AppendLine("    add.u64 %rd5, %rd1, %rd4;");
        ptx.AppendLine("    ld.global.s32 %r7, [%rd5];");
        ptx.AppendLine("    setp.ne.s32 %p1, %r7, %r3;");
        ptx.AppendLine("    @%p1 bra NEURAL_SCATTER_MAX_NEXT;");
        ...
        ptx.AppendLine("NEURAL_SCATTER_MAX_NEXT:");
        ptx.AppendLine("    add.u32 %r5, %r5, 1;");
        ptx.AppendLine("    bra NEURAL_SCATTER_MAX_LOOP;");
        ptx.AppendLine("NEURAL_SCATTER_MAX_DONE:");
        ...
        ptx.AppendLine("    st.global.s32 [%rd10], %r6;");
        ptx.AppendLine("    ret;");
        ptx.AppendLine("}");
        return ptx.ToString();
    }
