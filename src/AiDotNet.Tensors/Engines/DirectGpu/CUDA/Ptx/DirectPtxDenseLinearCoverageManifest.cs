...
internal static class DirectPtxDenseLinearCoverageManifest
{
    ...
    [
        Cell("CudaBackend.Gemm", "cuBLAS SGEMM", "C=alpha*A@B+beta*C", Dense, "FP32", PlannedGemm),
        ...
        Cell("CudaBackend.OuterProduct", "NVRTC outer-product kernel", "output=a outer b", "a[M], b[N], output[M,N]", "FP32", PlannedGemm),
        Cell("CudaBackend.DotProduct", "NVRTC reduction", "scalar sum(a*b)", "a[K], b[K], output[1]", "FP32", PlannedGemm),
