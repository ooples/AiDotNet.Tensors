...
internal static class DirectPtxDenseLinearCoverageManifest
{
    ...
    [
        ...
        Cell("CudaBackend.FusedLinearGELUTransposedM1", "direct PTX or MatMulTransposed+BiasAdd+Gelu", "gelu_tanh(x@transpose(W)+bias)", "x[K], W[N,K], bias/output[N]", "FP32", "v1 Ampere M=1 exact-shape PTX; all measured buckets remain experiment-only after the final release matrix", DirectPtxDenseLinearCoverageStatus.ExperimentalDirectPtx),
        ...
        Cell("CudaBackend.FusedLinearGELU", "NVRTC fused linear kernel", "gelu_tanh(input@weight+bias)", Dense + ", bias[N]", "FP32", "input-major exact-shape tiled PTX; M=1 output-major specialization is also experiment-only", DirectPtxDenseLinearCoverageStatus.ExperimentalDirectPtx),
