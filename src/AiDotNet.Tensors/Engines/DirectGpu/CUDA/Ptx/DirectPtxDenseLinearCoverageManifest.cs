...
internal static class DirectPtxDenseLinearCoverageManifest
{
    ...
    [
        ...
        Cell("DirectGpuTensorEngine.TensorMatMul", "backend Gemm/BatchedGemm/Hgemm routing", "public rank-aware matrix product", "canonical contiguous 2D/3D tensors", "generic public; GPU FP16/FP32", "backend Gemm (the 2D FP32 alpha=1/beta=0 MatMul path) routes through the plain PTX tile via a fail-closed TryDirectPtxGemm guard (returns false until GPU-promoted), then falls through to cuBLAS", DirectPtxDenseLinearCoverageStatus.ExperimentalDirectPtx),
        ...
        Cell("DirectGpuTensorEngine.FusedLinearBackward", "backend fused backward or baseline", "public linear activation backward", "canonical forward tensors plus gradients", "generic public; GPU FP32", "all five backend FusedLinear*Backward methods route through the four-kernel PTX composite (dZ + dInput + dWeight + dBias) via a fail-closed guard: ReLU/GELU/Swish use the preactivation-form dZ, sigmoid/tanh the post-activation-form dZ (y(1-y) / 1-y^2)", DirectPtxDenseLinearCoverageStatus.ExperimentalDirectPtx),
        Cell("DirectGpuTensorEngine.FusedLoRAForward", "CudaBackend.FusedLoRAForward or CPU fallback", "public LoRA forward", "input[M,K], base[M,N], A[K,R], B[R,N]", "FP32", "backend FusedLoRAForward routes the r in {16,32,64} standard-layout case through the single-launch PTX kernel (PtxFusedLoRAForwardStandardKernel, MaxRank-strided resident Z with a bounds-guarded loraA load) via a fail-closed TryDirectPtxFusedLoRAForward guard (returns false until GPU-promoted), then falls through to the NVRTC kernel", DirectPtxDenseLinearCoverageStatus.ExperimentalDirectPtx)
    ];
