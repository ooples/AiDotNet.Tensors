...
internal static class DirectPtxDenseLinearCoverageManifest
{
    ...
    [
        ...
        Cell("DirectGpuTensorEngine.FusedLinearBackward", "backend fused backward or baseline", "public linear activation backward", "canonical forward tensors plus gradients", "generic public; GPU FP32", "backend FusedLinear{ReLU,GELU,Swish}Backward (the preactivation-form activations) route through the four-kernel PTX composite (dZ + dInput + dWeight + dBias) via a fail-closed guard; sigmoid/tanh remain baseline pending a post-activation-form derivative kernel", DirectPtxDenseLinearCoverageStatus.ExperimentalDirectPtx),
        Cell("DirectGpuTensorEngine.FusedLoRAForward", "CudaBackend.FusedLoRAForward or CPU fallback", "public LoRA forward", "input[M,K], base[M,N], A[K,R], B[R,N]", "FP32", "backend FusedLoRAForward routes the r=64 standard-layout case through the single-launch PTX kernel (PtxFusedLoRAForwardStandardKernel) via a fail-closed TryDirectPtxFusedLoRAForward guard (returns false until GPU-promoted), then falls through to the NVRTC kernel", DirectPtxDenseLinearCoverageStatus.ExperimentalDirectPtx)
    ];
