...
internal sealed class PtxDeformableConv2DBackwardInputKernel : IDisposable
{
    ...
    internal string EntryPoint => FormattableString.Invariant(
        $"aidotnet_deformable_conv2d_bwd_input_n{Batch}_c{InputChannels}_k{OutputChannels}_h{Height}_w{Width}_kh{KernelH}_kw{KernelW}_s{Stride}_p{Padding}");
    ...
    internal string EmitPtx(int major, int minor)
    {
        ...
        int offN = 2 * taps * ohow, maskN = taps * ohow, total = TotalThreads;
