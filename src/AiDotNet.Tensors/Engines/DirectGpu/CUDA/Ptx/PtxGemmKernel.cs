...
/// <summary>
/// Plain FP32 GEMM <c>C[M,N] = A[M,K] @ B[K,N]</c> (no bias, no activation), the
/// bare-epilogue tile behind Gemm / GemmAsync / MatMul. Same register-blocked
/// shared-memory tile as <see cref="PtxFusedGemmBiasKernel"/> (BM=BN=64, BK=8,
/// TM=TN=4 → 256 threads, 16 accumulators, 4 KiB shared); Grid = (N/BN, M/BM).
/// </summary>
internal sealed class PtxGemmKernel : IDisposable
{
    ...
    internal const string EntryPoint = "aidotnet_gemm";
    ...
    internal PtxGemmKernel(DirectPtxRuntime runtime, int m, int k, int n)
    {
        PtxCompat.ThrowIfNull(runtime, nameof(runtime));
        if (!DirectPtxArchitecture.HasValidatedFusedLinear(
            runtime.ComputeCapabilityMajor, runtime.ComputeCapabilityMinor))
            throw new PlatformNotSupportedException(
                "The checked-in GEMM specialization is measured only on GA10x/SM86.");
        ValidateShape(m, k, n);
        M = m;
        K = k;
        N = n;
    ...
    internal unsafe void Launch(DirectPtxTensorView a, DirectPtxTensorView b, DirectPtxTensorView output)
    {
        ...
        if (Overlaps(output, a) || Overlaps(output, b))
            throw new ArgumentException("GEMM output may not alias A or B.");
    ...
    internal static string EmitPtx(int ccMajor, int ccMinor, int m, int k, int n)
    {
        ...
        ptx.AppendLine($"// gemm M={m} K={k} N={n} tile={BlockM}x{BlockN}x{BlockK} thread={ThreadM}x{ThreadN}");
        ptx.AppendLine();
    ...
    private static void ValidateShape(int m, int k, int n)
    {
        if (!IsSupportedShape(m, k, n))
            throw new ArgumentOutOfRangeException(
                nameof(m),
                "GEMM supports M in {64,128,256,512,1024,2048}, K/N in {256,512,1024,2048,4096}.");
    }
