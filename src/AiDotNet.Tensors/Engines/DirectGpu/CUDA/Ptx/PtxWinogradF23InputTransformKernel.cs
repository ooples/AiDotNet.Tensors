...
/// <summary>
/// Winograd F(2,3) input transform: for every (input-channel c, output-tile p)
/// gathers the 4x4 same-padded input patch d and writes V = B^T d B into a
/// position-major buffer V[16, C, P] (V[xi*C*P + c*P + p]), where P = N*TH*TW is
/// the number of 2x2 output tiles. Computing V once per (c, tile) removes the
/// K-fold redundant input load/transform of the correctness-first kernel; the
/// batched GEMM then reads V. One thread per (c, p).
/// </summary>
internal sealed class PtxWinogradF23InputTransformKernel : IDisposable
{
    ...
    internal long TransformedBytes => (long)16 * InputChannels * Tiles * sizeof(float);   // V[16,C,P]

    internal string EntryPoint => FormattableString.Invariant(
        $"aidotnet_winograd_f23_input_transform_n{Batch}_c{InputChannels}_h{Height}_w{Width}");

    internal PtxWinogradF23InputTransformKernel(
        DirectPtxRuntime runtime, int batch, int inputChannels, int height, int width)
    {
        PtxCompat.ThrowIfNull(runtime, nameof(runtime));
        if (!DirectPtxArchitecture.HasExperimentalConvolution(
                runtime.ComputeCapabilityMajor, runtime.ComputeCapabilityMinor))
            throw new NotSupportedException("Winograd input transform has no experimental non-SM86 specialization.");
        ...
        DirectPtxArchitectureFamily architecture, int n, int c, int h, int w)
    {
        ...
        var v = new DirectPtxExtent(16, c, tiles);
        return new DirectPtxKernelBlueprint(
            Operation: "winograd-f23-input-transform",
            Version: 1,
            Architecture: architecture,
            Variant: FormattableString.Invariant($"n{n}-c{c}-h{h}-w{w}-winograd-f23-fp32"),
            Tensors:
            [
                ...
                new("transformed", DirectPtxPhysicalType.Float32, DirectPtxPhysicalLayout.RowMajor2D,
            ...
            {
                ["equation"] = "V = B^T d B",
                ["layout"] = "nchw -> position-major V[16,C,P]",
                ["padding"] = "same-1",
    ...
    internal static string EmitPtx(int major, int minor, int n, int c, int h, int w)
    {
        if (!DirectPtxArchitecture.HasExperimentalConvolution(major, minor))
            throw new NotSupportedException("Only the experimental SM86 input transform emitter exists.");
        string I(int v) => v.ToString(CultureInfo.InvariantCulture);
        int th = h / 2, tw = w / 2;
        int p = n * th * tw;        // P
        int cp = c * p;             // C*P (V position stride)
        int hw = h * w;
        int chw = c * h * w;
        string entry = FormattableString.Invariant(
            $"aidotnet_winograd_f23_input_transform_n{n}_c{c}_h{h}_w{w}");
        ...
        // store V[xi*C*P + c*P + pp] for xi=0..15. base = V + (c*P + pp)*4 ; stride between xi = C*P*4
        s.AppendLine($"    mad.lo.u32 %r15, %r4, {I(p)}, %r3;");             // c*P + pp
        s.AppendLine("    mul.wide.u32 %rd4, %r15, 4;");
        s.AppendLine("    add.u64 %rd4, %rd1, %rd4;");                       // &V[0][c][pp]
        for (int xi = 0; xi < 16; xi++)
            s.AppendLine($"    st.global.f32 [%rd4+{I(xi * cp * 4)}], %f{I(32 + xi)};");
        s.AppendLine("    ret;");
        s.AppendLine("}");
        return s.ToString();
    }
