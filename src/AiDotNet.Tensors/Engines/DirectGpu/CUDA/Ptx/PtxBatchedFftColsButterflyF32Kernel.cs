...
/// <summary>
/// One batched column-wise radix-2 DIT FFT butterfly stage for issue #850: the column pass of a batched 2D
/// FFT over <c>images</c> contiguous row-major <c>height x width</c> matrices. It extends
/// <see cref="PtxFftColsButterflyF32Kernel"/> with an image dimension carried by <c>gridDim.y</c>
/// (<c>%ctaid.y</c>), which offsets into image <c>img</c> at <c>img * height * width</c> so columns never
/// cross image boundaries. Each thread owns one <c>(image, column, wing)</c> cell; the wing selects a
/// top/bottom row pair spanning <c>halfStride</c> rows addressed with the column stride <c>width</c>.
/// <c>height</c> and <c>width</c> are baked; the per-stage <c>stride</c> and <c>inverse</c> flag are
/// per-launch <c>.param .u32</c> controls. The twiddle uses <c>cos.approx</c>/<c>sin.approx</c>
/// (tolerance-based parity). The launch covers exactly <c>images * width * (height/2)</c> wings, so no
/// bounds guard is emitted. Two pointers plus two u32 controls reach the launch ABI.
///
/// The specialization stays disabled by default and fails closed until three clean promotion runs clear
/// the release gate.
/// </summary>
internal sealed class PtxBatchedFftColsButterflyF32Kernel : IDisposable
{
    internal const int DefaultBlockThreads = 256;
    internal const string EntryPoint = "aidotnet_batched_fft_cols_butterfly_f32";
    ...
    internal PtxBatchedFftColsButterflyF32Kernel(
        DirectPtxRuntime runtime, int height, int width, int images, int blockThreads = DefaultBlockThreads)
    {
        ...
        if (!DirectPtxArchitecture.HasValidatedComplexMultiply(
            ...
            throw new PlatformNotSupportedException(
                "The checked-in batched column FFT butterfly specialization is admitted only on SM86.");
    ...
    /// row span (2,4,...,height); <paramref name="inverse"/> is 1 for the inverse transform, 0 for forward.</summary>
    internal unsafe void Launch(DirectPtxTensorView real, DirectPtxTensorView imag, int stride, int inverse)
    {
        ...
        _module.Launch(
            _function,
            checked((uint)((Width * (Height / 2)) / BlockThreads)), checked((uint)Images), 1,
    ...
    internal static string EmitPtx(int ccMajor, int ccMinor, int height, int width, int blockThreads = DefaultBlockThreads)
    {
        ...
        var ptx = new StringBuilder(4_864);
        ptx.AppendLine(".version 7.1");
        ptx.AppendLine($".target sm_{ccMajor}{ccMinor}");
        ptx.AppendLine(".address_size 64");
        ptx.AppendLine($"// exact-shape height={height} width={width} block={blockThreads} op=batched-fft-cols-butterfly (image=gridDimY; stride,inverse are .param)");
        ptx.AppendLine();
        ...
        ptx.AppendLine("    mad.lo.u32 %r10, %r6, %r8, %r7;");            // topRow
        ptx.AppendLine("    mov.u32 %r14, %ctaid.y;");                     // image index
        ptx.AppendLine($"    mul.lo.u32 %r15, %r14, {imageStride};");     // imgOffset
        ptx.AppendLine($"    mad.lo.u32 %r11, %r10, {width}, %r3;");      // topIdx = topRow*width + col
        ptx.AppendLine("    add.u32 %r11, %r11, %r15;");                  // + imgOffset
        ...
        DirectPtxArchitectureFamily architecture, int height, int width, int images, int blockThreads)
    {
        ...
        return new DirectPtxKernelBlueprint(
            Operation: "batched-fft-cols-butterfly-f32",
            Version: 1,
            ...
            {
                ["formula"] = "per image, column col: top'=top+W*bot, bot'=top-W*bot at column stride width plus img offset, W=exp(+/-2pi i wing/stride)",
                ["mode"] = "inference-forward-batched-fft-cols-butterfly-stage",
                ...
                ["layout"] = "row-major images x height x width; column stride = width; bottom row = top row + halfStride",
                ["bounds-check"] = "none - the launch covers exactly images*width*(height/2) wings",
    ...
    private static void Validate(int height, int width, int images)
    {
        if (!IsSupportedShape(height, width, images))
            throw new ArgumentOutOfRangeException(nameof(height),
                "The batched column FFT butterfly family supports power-of-two height,width>=2 with " +
                "width*(height/2)>=256 a multiple of 256 (up to height*width=2^24), images>=1, and " +
                "images*height*width<=2^26.");
    }
    ...
    private static void ValidateShape(int height, int width)
    {
        if (!ShapeOk(height, width))
            throw new ArgumentOutOfRangeException(nameof(height),
                "The batched column FFT butterfly family supports power-of-two height,width>=2 with " +
                "width*(height/2)>=256 a multiple of 256, up to height*width=2^24.");
    }
    ...
    private static void ValidateBlockThreads(int height, int width, int blockThreads)
    {
        if (blockThreads is not (128 or 256 or 512) || (width * (height / 2)) % blockThreads != 0)
            throw new ArgumentOutOfRangeException(nameof(blockThreads),
                "Batched column FFT butterfly block threads must be 128, 256, or 512 and evenly tile width*(height/2).");
    }
