...
internal static class DirectPtxGatherCoverageManifest
{
    ...
    [
        ...
        Experimental("CudaBackend.IndexSelect", "NVRTC index_select", "select rows by index along an axis", Indexed, "FP32 rows; INT32 indices", "v1 Ampere linear-element exact-shape cells; FLOAT index buffer with a truncating (int) cast - a different contract from the int-index gather, so a separate module"),
        ...
        Planned("CudaBackend.GatherBackward", "none - no backend op exists", "scatter-add output gradient to source rows", Indexed, "FP32 rows; INT32 indices", "blocked: needs a public backend op first, then atomic-scatter-add-row-families"),
        Planned("CudaBackend.ScatterRows", "none - no backend op exists", "output[index[i],:] = source[i,:]", Indexed, "FP32 rows; INT32 indices", "blocked: needs a public backend op first, then warp-row-scatter-exact-shape-families"),
        Planned("CudaBackend.ScatterAddRows", "none - no backend op exists", "output[index[i],:] += source[i,:]", Indexed, "FP32 rows; INT32 indices", "blocked: needs a public backend op first, then atomic-scatter-add-row-families"),
        ...
        Planned("CudaBackend.GatherNd", "none - no backend op exists", "gather by multi-dimensional coordinate", "canonical contiguous coordinate/output rows", "FP32 rows; INT32 coordinates", "blocked: needs a public backend op first, then baked-rank-coordinate-gather-families"),
        Planned("CudaBackend.GatherAxis", "none - no backend op exists", "gather along an arbitrary axis", "canonical contiguous input/index/output with baked axis stride", "FP32 rows; INT32 indices", "blocked: needs a public backend op first, then baked-axis-stride-gather-families"),
        Planned("CudaBackend.TakeAlongAxis", "none - no backend op exists", "elementwise index along an axis", Rows, "FP32 rows; INT32 indices", "blocked: needs a public backend op first, then warp-row-take-along-axis-families"),
        Planned("CudaBackend.OneHot", "none - no backend op exists", "index to baked-width one-hot rows", Indexed, "INT32 indices; FP32 output", "blocked: needs a public backend op first, then baked-width-one-hot-families"),
        Planned("CudaBackend.SegmentSum", "none - no backend op exists", "sum source rows within contiguous segments", "canonical contiguous source/segment-offset rows", "FP32 rows; INT32 offsets", "blocked: needs a public backend op first, then segmented-scan-offset-families"),
        Planned("CudaBackend.SegmentMean", "none - no backend op exists", "mean source rows within contiguous segments", "canonical contiguous source/segment-offset rows", "FP32 rows; INT32 offsets", "blocked: needs a public backend op first, then segmented-scan-offset-families"),
        Planned("CudaBackend.SparseUpdateRows", "none - no backend op exists", "apply a sparse row update set to a dense table", Indexed, "FP32 rows; INT32 indices", "blocked: needs a public backend op first, then atomic-or-exclusive-sparse-update-families"),
        Planned("CudaBackend.MaskedSelect", "none - no backend op exists", "compact rows where a boolean mask is set", "canonical contiguous input/mask/output rows", "FP32 rows; boolean mask", "blocked: needs a public backend op first, then prefix-sum-compaction-families"),
