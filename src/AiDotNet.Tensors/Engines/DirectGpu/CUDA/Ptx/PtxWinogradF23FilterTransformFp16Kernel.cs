...
/// <summary>
/// FP16 Winograd F(2,3) filter transform for the Tensor-Core GEMM path: reads
/// OIHW weights[K,C,3,3] (fp32), computes U = G g G^T in fp32, and writes the 16
/// transformed positions as fp16 into a position-major buffer U[16, K, C]
/// (U[xi*K*C + k*C + c]). This is exactly the A operand layout the WMMA batched
/// GEMM consumes (per position xi, U[xi] is a row-major [K, C] matrix). One
/// thread per (k, c). Run once — filters are constant.
/// </summary>
internal sealed class PtxWinogradF23FilterTransformFp16Kernel : IDisposable
{
    ...
    internal string EntryPoint =>
        FormattableString.Invariant($"aidotnet_winograd_f23_filter_transform_fp16_k{OutputChannels}_c{InputChannels}");
    internal long WeightBytes => (long)OutputChannels * InputChannels * 9 * sizeof(float);
    internal long TransformedBytes => (long)16 * OutputChannels * InputChannels * sizeof(ushort);

    internal PtxWinogradF23FilterTransformFp16Kernel(DirectPtxRuntime runtime, int outputChannels, int inputChannels)
    {
        PtxCompat.ThrowIfNull(runtime, nameof(runtime));
        if (!DirectPtxArchitecture.HasExperimentalConvolution(
                runtime.ComputeCapabilityMajor, runtime.ComputeCapabilityMinor))
            throw new NotSupportedException("Winograd fp16 filter transform has no experimental non-SM86 specialization.");
        ...
        DirectPtxArchitectureFamily architecture, int k, int c)
    {
        ...
        return new DirectPtxKernelBlueprint(
            Operation: "winograd-f23-filter-transform-fp16",
            Version: 1,
            Architecture: architecture,
            Variant: FormattableString.Invariant($"k{k}-c{c}-r3-winograd-f23-fp16"),
            Tensors:
            [
            ...
            {
                ["equation"] = "U = G g G^T",
                ["input"] = "fp32", ["output"] = "fp16", ["layout"] = "oihw -> position-major U[16,K,C]",
    ...
    internal static string EmitPtx(int major, int minor, int k, int c)
    {
        if (!DirectPtxArchitecture.HasExperimentalConvolution(major, minor))
            throw new NotSupportedException("Only the experimental SM86 fp16 filter transform emitter exists.");
        string I(int v) => v.ToString(CultureInfo.InvariantCulture);
        string entry = FormattableString.Invariant($"aidotnet_winograd_f23_filter_transform_fp16_k{k}_c{c}");
        int total = k * c;
        int xiStride = total * sizeof(ushort);      // fp16 xi stride: K*C*2
        ...
        // U position-major fp16: base = transformed + id*2 (slot in xi=0); xi stride = K*C*2.
        p.AppendLine("    mul.wide.u32 %rd3, %r2, 2;");
        ...
        // Cast each of the 16 positions to fp16 and store at xi stride.
        for (int i = 0; i < 16; i++)
        {
            p.AppendLine($"    cvt.rn.f16.f32 %h0, %f{I(21 + i)};");
            p.AppendLine($"    st.global.b16 [%rd3+{I(i * xiStride)}], %h0;");
        }
        p.AppendLine("DONE:");
        p.AppendLine("    ret;");
        p.AppendLine("}");
