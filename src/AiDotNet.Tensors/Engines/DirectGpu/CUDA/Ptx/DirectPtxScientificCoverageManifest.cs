...
internal static class DirectPtxScientificCoverageManifest
{
    ...
    [
        ...
        new("CudaBackend.PoincareProject", "NVRTC poincare project", "project onto the Poincare ball", "Poincare vectors", "FP32", DirectPtxScientificCoverageStatus.ExperimentalDirectPtx, "warp-per-vector PTX kernel (PtxPoincareProjectKernel) with a shuffle norm and lane-0 scale broadcast; fails closed until GPU-validated and promoted"),
        ...
        new("CudaBackend.PoincareDistance", "NVRTC poincare distance", "hyperbolic geodesic distance", "Poincare vectors", "FP32", DirectPtxScientificCoverageStatus.ExperimentalDirectPtx, "one-vector-per-warp PTX kernel (PtxPoincareDistanceKernel) with four shuffle reductions, no block barriers, reused Mobius coefficients, and arctanh via lg2; fails closed until GPU-validated and promoted"),
        new("CudaBackend.OctonionMultiply", "NVRTC octonion multiply", "8-component hypercomplex product", "octonion 8-tuples", "FP32", DirectPtxScientificCoverageStatus.ExperimentalDirectPtx, "register-resident Cayley-Dickson PTX kernel (PtxOctonionMultiplyKernel) with aligned v4 transfers and signed FMA chains; fails closed until GPU-validated and promoted"),
        ...
        new("CudaBackend.RbfForward", "NVRTC rbf_forward", "exp(-epsilon_c * ||x_b - center_c||^2)", "row-major [batch,dim] / [centers,dim]", "FP32", DirectPtxScientificCoverageStatus.ExperimentalDirectPtx, "warp-per-(batch,center) coalesced-dimension PTX kernel (PtxRbfForwardKernel) with shuffle reduction and expf via ex2.approx; fails closed until GPU-validated and promoted"),
        new("CudaBackend.PairwiseDistance", "NVRTC pairwise_distance", "output[i,j] = ||a[i] - b[j]||", "row-major [M,dim] / [N,dim]", "FP32", DirectPtxScientificCoverageStatus.ExperimentalDirectPtx, "warp-per-(i,j) coalesced-dimension PTX kernel (PtxPairwiseDistanceKernel, L2 variant) with shuffle reduction and sqrt.rn.f32; fails closed until GPU-validated and promoted"),
        new("CudaBackend.PairwiseDistanceSquared", "NVRTC pairwise_distance_squared", "output[i,j] = ||a[i] - b[j]||^2", "row-major [M,dim] / [N,dim]", "FP32", DirectPtxScientificCoverageStatus.ExperimentalDirectPtx, "warp-per-(i,j) coalesced-dimension PTX kernel (PtxPairwiseDistanceKernel, squared variant) with shuffle reduction; fails closed until GPU-validated and promoted"),
        ...
        new("CudaBackend.ComplexMatVec", "NVRTC complex_matvec", "y[b] = A * x[b] over complex (shared [dim,dim] matrix)", "split real/imag; [dim,dim] matrix, [batch,dim] vectors", "FP32", DirectPtxScientificCoverageStatus.ExperimentalDirectPtx, "warp-per-(batch,row) coalesced-column PTX kernel (PtxComplexMatVecKernel) with two shuffle reductions and complex FMA accumulation; fails closed until GPU-validated and promoted"),
        new("CudaBackend.SphericalHarmonics", "NVRTC spherical_harmonics", "clamp01(sum_b coeff[i,b,ch] * SH_basis_b(dir_i))", "row-major coeff [points,basis,channels]; dir [points,3]", "FP32", DirectPtxScientificCoverageStatus.ExperimentalDirectPtx, "shape-selected scalar/four-channel PTX kernel (PtxSphericalHarmonicsKernel): register-resident degree-0..3 real SH basis + vector coefficient loads + unrolled dot-products + clamp01; fails closed until GPU-validated and promoted"),
        ...
        new("CudaBackend.CapsulePredictions", "NVRTC capsule_predictions", "output[b,i,c,d] = sum_k input[b,i,k] * weights[i,c,k,d]", "row-major input [B,I,K]; weights [I,C,K,D]", "FP32/TF32", DirectPtxScientificCoverageStatus.ExperimentalDirectPtx, "shape-selected TF32 WMMA 16x16 tile or scalar/x4/x8 FP32 serial-contraction PTX kernel (PtxCapsuleContractionKernel, Predictions variant); fails closed until GPU-validated and promoted"),
        new("CudaBackend.CapsuleTransform", "NVRTC capsule_transform", "output[b,i,j,d] = sum_k input[b,i,k] * weights[i,k,j,d]", "row-major input [B,I,K]; weights [I,K,J,D]", "FP32/TF32", DirectPtxScientificCoverageStatus.ExperimentalDirectPtx, "shape-selected TF32 WMMA 16x16 tile or scalar/x4/x8 FP32 serial-contraction PTX kernel (PtxCapsuleContractionKernel, Transform variant); fails closed until GPU-validated and promoted"),
        new("CudaBackend.CapsuleWeightedSum", "NVRTC capsule_weighted_sum", "output[b,c,d] = sum_i coupling[b,i,c] * predictions[b,i,c,d]", "row-major coupling [B,I,C]; predictions [B,I,C,D]", "FP32", DirectPtxScientificCoverageStatus.ExperimentalDirectPtx, "shape-selected scalar/x4/x8 output-coarsened serial-i PTX kernel (PtxCapsuleWeightedSumKernel); fails closed until GPU-validated and promoted"),
        new("CudaBackend.CapsuleAgreement", "NVRTC capsule_agreement", "agreement[b,i,c] = sum_d predictions[b,i,c,d] * output[b,c,d]", "row-major predictions [B,I,C,D]; output [B,C,D]", "FP32", DirectPtxScientificCoverageStatus.ExperimentalDirectPtx, "thread-per-output-element vectorized-d reduction PTX kernel (PtxCapsuleAgreementKernel) with shape-selected scalar/v2/v4 loads; fails closed until GPU-validated and promoted"),
        new("CudaBackend.CosineSimilarity", "NVRTC cosine_similarity", "output[b] = dot(a[b],b[b]) / (||a[b]||*||b[b]|| + 1e-8)", "row-major [B,dim] pairs", "FP32", DirectPtxScientificCoverageStatus.ExperimentalDirectPtx, "one-warp-per-row PTX kernel (PtxCosineSimilarityKernel) with coalesced lane-strided loads and three shuffle reductions; fails closed until GPU-validated and promoted"),
        ...
        new("CudaBackend.NormalizeProbabilities", "NVRTC normalize_probabilities", "probabilities[b,i] /= max(row-sum, 1e-10)", "row-major [batch,state]", "FP32", DirectPtxScientificCoverageStatus.ExperimentalDirectPtx, "one-warp-per-row shuffle-reduction PTX kernel (PtxNormalizeProbabilitiesKernel), in-place; fails closed until GPU-validated and promoted"),
        new("CudaBackend.MeasurementForward", "NVRTC measurement_forward", "output[b,i] = |z_i|^2 / max(sum_i |z_i|^2, 1e-10)", "interleaved complex input [batch,state,2]; output [batch,state]", "FP32", DirectPtxScientificCoverageStatus.ExperimentalDirectPtx, "one-warp-per-row shuffle-reduction PTX kernel (PtxMeasurementForwardKernel) fusing |z|^2 evaluation and normalization; fails closed until GPU-validated and promoted"),
        new("CudaBackend.QuantumRotation", "NVRTC quantum_rotation", "apply Ry(angles[q]) to each qubit q in turn on the split-complex state", "split real/imag [batch,2^numQubits]; angles [numQubits]", "FP32", DirectPtxScientificCoverageStatus.ExperimentalDirectPtx, "one-block-per-batch PTX kernel (PtxQuantumRotationKernel) with block-cached gate trig, shared-state staging for eligible rows, and dependency-scoped warp/block barriers between unrolled butterfly steps; fails closed until GPU-validated and promoted"),
        new("CudaBackend.AnnComputeDistances", "NVRTC ann_compute_distances", "distances[q,j] = metric(query_q, db_j); L2-squared or inner-product", "row-major queries [Q,dim]; database [DB,dim]", "FP32", DirectPtxScientificCoverageStatus.ExperimentalDirectPtx, "warp-per-cell coalesced-dimension PTX kernel (PtxAnnComputeDistancesKernel) with shuffle reduction and baked metric; fails closed until GPU-validated and promoted"),
        new("CudaBackend.AnnPqDistanceTables", "NVRTC ann_pq_distance_tables", "tables[q,s,c] = metric(query subvec, codebook subcentroid)", "row-major queries [Q,m*dsub]; codebooks [m,ksub,dsub]", "FP32", DirectPtxScientificCoverageStatus.ExperimentalDirectPtx, "thread-per-cell vectorized-subdimension PTX kernel (PtxAnnPqDistanceTablesKernel) with shape-selected scalar/v2/v4 loads and baked metric; fails closed until GPU-validated and promoted"),
        new("CudaBackend.AnnIvfAssign", "NVRTC ann_ivf_assign", "assignments[i] = argmin/argmax_c metric(vector_i, centroid_c), ties to lowest c", "row-major vectors [V,dim]; centroids [C,dim]; int32 assignments", "FP32/INT32", DirectPtxScientificCoverageStatus.ExperimentalDirectPtx, "warp-per-vector coalesced centroid-scan PTX kernel (PtxAnnIvfAssignKernel) with shuffle reduction, baked metric, strict-improvement argmin/argmax and int32 output; fails closed until GPU-validated and promoted"),
        new("CudaBackend.AnnPqAdcScan", "NVRTC ann_pq_adc_scan", "distances[q,i] = sum_s tables[q, s, codes[i,s]]", "uint8 codes [codes,m]; tables [Q,m,ksub]; distances [Q,codes]", "FP32/UINT8", DirectPtxScientificCoverageStatus.ExperimentalDirectPtx, "thread-per-cell PTX kernel (PtxAnnPqAdcScanKernel) with x8 strip-mined packed-u32 code extraction and FP32 table gathers; fails closed until GPU-validated and promoted"),
