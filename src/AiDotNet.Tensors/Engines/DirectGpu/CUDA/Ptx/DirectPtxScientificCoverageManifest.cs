...
internal static class DirectPtxScientificCoverageManifest
{
    ...
    [
        ...
        new("CudaBackend.CapsulePredictions", "NVRTC capsule_predictions", "output[b,i,c,d] = sum_k input[b,i,k] * weights[i,c,k,d]", "row-major input [B,I,K]; weights [I,C,K,D]", "FP32", DirectPtxScientificCoverageStatus.ExperimentalDirectPtx, "shape-selected scalar/x4/x8 output-coarsened serial-contraction PTX kernel (PtxCapsuleContractionKernel, Predictions variant); fails closed until GPU-validated and promoted"),
        new("CudaBackend.CapsuleTransform", "NVRTC capsule_transform", "output[b,i,j,d] = sum_k input[b,i,k] * weights[i,k,j,d]", "row-major input [B,I,K]; weights [I,K,J,D]", "FP32", DirectPtxScientificCoverageStatus.ExperimentalDirectPtx, "shape-selected scalar/x4/x8 output-coarsened serial-contraction PTX kernel (PtxCapsuleContractionKernel, Transform variant); fails closed until GPU-validated and promoted"),
