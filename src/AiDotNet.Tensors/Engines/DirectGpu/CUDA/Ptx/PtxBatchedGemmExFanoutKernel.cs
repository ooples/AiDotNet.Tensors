...
/// <summary>
/// Mixed-precision batched pointer-fanout GEMM (the GemmEx fanout path): a single shared
/// FP16 <c>A[M,K]</c> multiplied against a batch of FP16 <c>B_i[K,N]</c> producing FP16
/// <c>C_i[M,N] = A @ B_i</c> with FP32 accumulation, where the B_i / C_i bases are
/// supplied as device pointer arrays and resolved per block from <c>%ctaid.z</c>. Combines
/// the pointer-fanout ABI of <see cref="PtxBatchedGemmFanoutKernel"/> with the FP16
/// convert-in / FP32-accumulate / convert-out path of <see cref="PtxHgemmKernel"/>.
///
/// Tile BM=BN=64, BK=8, TM=TN=4 → 256 threads, 16 FP32 accumulators/thread, 4 KiB
/// shared. Grid = (N/BN, M/BM, batch).
/// </summary>
/// <remarks>
/// INTEGRATION STATUS (2026-09-01): no <c>CudaBackend</c> entry point constructs this kernel; BatchedGemmExFanout is not served by any direct route; only the driver tests construct this kernel.
/// Until a route dispatches it, its emitted PTX is exercised by tests only and it carries no parity or
/// performance claim.
/// </remarks>
internal sealed class PtxBatchedGemmExFanoutKernel : IDisposable
{
    ...
    internal const string EntryPoint = "aidotnet_batched_gemm_ex_fanout";
    ...
    internal PtxBatchedGemmExFanoutKernel(DirectPtxRuntime runtime, int m, int k, int n, int batch)
    {
        PtxCompat.ThrowIfNull(runtime, nameof(runtime));
        if (!DirectPtxArchitecture.HasValidatedFusedLinear(
            runtime.ComputeCapabilityMajor, runtime.ComputeCapabilityMinor))
            throw new PlatformNotSupportedException(
                "The checked-in mixed-precision batched-fanout specialization is measured only on GA10x/SM86.");
    ...
    /// <summary>
    /// Launches the mixed-precision fanout. <paramref name="a"/> is the shared FP16 left
    /// operand; the pointer arrays are device buffers each holding <see cref="Batch"/>
    /// little-endian 64-bit device addresses (FP16 B_i / C_i bases).
    /// </summary>
    ...
    internal static string EmitPtx(int ccMajor, int ccMinor, int m, int k, int n)
    {
        ...
        ptx.AppendLine($"// batched-gemm-ex-fanout M={m} K={k} N={n} tile={BlockM}x{BlockN}x{BlockK}");
        ptx.AppendLine();
        ...
        ptx.AppendLine("    ld.global.nc.u64 %rd1, [%rd23];");         // FP16 B_z base
        ptx.AppendLine("    add.u64 %rd23, %rd21, %rd22;");
        ptx.AppendLine("    ld.global.nc.u64 %rd3, [%rd23];");         // FP16 C_z base
    ...
    // Standard-B FP16 load: A[m,k] and B[k,n], each converted from FP16 to the FP32 tile.
    ...
    // FP16 output C_z: narrow each accumulator and store 2 bytes.
        ...
        DirectPtxArchitectureFamily architecture, int m, int k, int n, int batch)
    {
        ...
        return new DirectPtxKernelBlueprint(
            Operation: "batched-gemm-ex-fanout",
            Version: 1,
            Architecture: architecture,
            Variant: $"fp16-m{m}-k{k}-n{n}-batch{batch}",
            Tensors:
            [
                ...
                new("b_element", DirectPtxPhysicalType.Float16, DirectPtxPhysicalLayout.RowMajor2D,
                    b, b, 16, DirectPtxTensorAccess.Read, DirectPtxExtentMode.Exact),
                new("c_element", DirectPtxPhysicalType.Float16, DirectPtxPhysicalLayout.RowMajor2D,
            ...
            {
                ["formula"] = "C_i_fp16[M,N] = A_fp16[M,K] @ B_i_fp16[K,N] for i in [0,batch)",
                ["batch-abi"] = "device-pointer-arrays-for-B-and-C-indexed-by-ctaid-z",
                ["precision"] = "fp16-in-fp32-accumulate-fp16-out",
    ...
    private static void ValidateShape(int m, int k, int n, int batch)
    {
        if (!IsSupportedShape(m, k, n, batch))
            throw new ArgumentOutOfRangeException(
                nameof(m),
                "Mixed-precision batched fanout supports M in {64,128,256,512,1024,2048}, K/N in {256,512,1024,2048,4096}, batch 1..256.");
    }
