...
/// <summary>
/// General-M FP32 <c>C[M,N] = A[M,K] @ transpose(B[N,K])</c> for the
/// <c>MatMulTransposed</c> entry point: A is row-major and B is canonical
/// output-major (row n contiguous over K, identical to the fused-linear weight
/// contract), so both operands stage as row-major BM×BK / BN×BK tiles and no
/// on-the-fly transpose or global scratch is needed. It is the register-blocked
/// tiled GEMM of <see cref="PtxFusedLinearTiledKernel"/> with the bias load and
/// activation epilogue removed — a bare accumulator store.
///
...
internal sealed class PtxMatMulTransposedKernel : IDisposable
{
    ...
    internal const string EntryPoint = "aidotnet_matmul_transposed";
    ...
    internal PtxMatMulTransposedKernel(DirectPtxRuntime runtime, int m, int k, int n)
    {
        PtxCompat.ThrowIfNull(runtime, nameof(runtime));
        if (!DirectPtxArchitecture.HasValidatedFusedLinear(
            runtime.ComputeCapabilityMajor, runtime.ComputeCapabilityMinor))
            throw new PlatformNotSupportedException(
                "The checked-in MatMulTransposed specialization is measured only on GA10x/SM86.");
        ValidateShape(m, k, n);
        M = m;
        K = k;
        N = n;
    ...
    internal unsafe void Launch(
        DirectPtxTensorView a, DirectPtxTensorView b, DirectPtxTensorView output)
    {
        ...
        if (Overlaps(output, a) || Overlaps(output, b))
            throw new ArgumentException("MatMulTransposed output may not alias A or B.");
    ...
    internal static string EmitPtx(int ccMajor, int ccMinor, int m, int k, int n)
    {
        ...
        ptx.AppendLine(
            $"// matmul-transposed M={m} K={k} N={n} tile={BlockM}x{BlockN}x{BlockK} thread={ThreadM}x{ThreadN}");
        ptx.AppendLine();
    ...
    // Each thread loads 2 A elements and 2 B elements into shared memory (both
    // row-major over BK; A rows index M, B rows index N).
    private static void EmitCooperativeLoad(StringBuilder ptx, int kBytes)
    {
        for (int slot = 0; slot < 2; slot++)
        {
            ...
            // B: output-major row over N; globalRow = blockCol*BN + row = %r6 + row.
    ...
    // Bare store — no bias, no activation.
        ...
        DirectPtxArchitectureFamily architecture, int m, int k, int n)
    {
        ...
        var b = new DirectPtxExtent(n, k);
        var output = new DirectPtxExtent(m, n);
        return new DirectPtxKernelBlueprint(
            Operation: "matmul-transposed",
            Version: 1,
            Architecture: architecture,
            Variant: $"fp32-m{m}-k{k}-n{n}",
            Tensors:
            [
                ...
                new("b", DirectPtxPhysicalType.Float32, DirectPtxPhysicalLayout.LinearWeightOutputMajor,
            ...
            {
                ["formula"] = "C[M,N] = A[M,K] @ transpose(B[N,K])",
                ["b-layout"] = "output-major-row-major-fp32",
    ...
    private static void ValidateShape(int m, int k, int n)
    {
        if (!IsSupportedShape(m, k, n))
            throw new ArgumentOutOfRangeException(
                nameof(m),
                "MatMulTransposed supports M in {64,128,256,512,1024,2048}, K/N in {256,512,1024,2048,4096}.");
    }
