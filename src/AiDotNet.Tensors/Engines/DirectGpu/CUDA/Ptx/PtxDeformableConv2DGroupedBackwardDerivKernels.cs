...
internal sealed class PtxDeformableConv2DGroupedBackwardOffsetKernel : IDisposable
{
    ...
    internal string EntryPoint => FormattableString.Invariant(
        $"aidotnet_deform_grouped_bwd_offset_n{Batch}_c{InputChannels}_k{OutputChannels}_h{Height}_w{Width}_kh{KernelH}_kw{KernelW}_s{Stride}_p{Padding}_dg{DeformGroups}");
...
internal sealed class PtxDeformableConv2DGroupedBackwardMaskKernel : IDisposable
{
    ...
    internal string EntryPoint => FormattableString.Invariant(
        $"aidotnet_deform_grouped_bwd_mask_n{Batch}_c{InputChannels}_k{OutputChannels}_h{Height}_w{Width}_kh{KernelH}_kw{KernelW}_s{Stride}_p{Padding}_dg{DeformGroups}");
    ...
    internal string EmitPtx(int major, int minor)
    {
        ...
        int taps = KernelH * kw, hw = h * w, chw = c * hw, ohow = ohh * oww, kohow = k * ohow, ckk = c * taps;
        int maskNg = DeformGroups * taps * ohow, offNg = DeformGroups * 2 * taps * ohow;
        int offGroup = 2 * taps * ohow, cpg = ChannelsPerGroup, total = TotalThreads;
        string entry = EntryPoint;
