...
/// <summary>
/// Elementwise complex magnitude <c>magnitude[i] = sqrt(real[i]^2 + imag[i]^2)</c> from
/// separate real/imag buffers (issue #854). One thread owns one element — no shared memory,
/// no reduction. Uses <c>sqrt.rn.f32</c> (IEEE round-to-nearest), so the result is exact to
/// FP32 rounding. 256 threads/block, grid = count/256 (positive multiple of 256).
/// </summary>
internal sealed class PtxComplexMagnitudeKernel : IDisposable
{
    internal const int BlockThreads = 256;
    internal const int MaxCount = 2048 * 4096;
    internal const string EntryPoint = "aidotnet_complex_magnitude";
    ...
    internal PtxComplexMagnitudeKernel(DirectPtxRuntime runtime, int count)
    {
        PtxCompat.ThrowIfNull(runtime, nameof(runtime));
        if (!DirectPtxArchitecture.HasValidatedScientific(
            runtime.ComputeCapabilityMajor, runtime.ComputeCapabilityMinor))
            throw new PlatformNotSupportedException(
                "The checked-in complex-magnitude specialization is measured only on GA10x/SM86.");
        ValidateShape(count);
        Count = count;
        ...
        DirectPtxTensorView real, DirectPtxTensorView imag, DirectPtxTensorView magnitude)
    {
        ...
        IntPtr magPointer = magnitude.Pointer;
    ...
    internal static string EmitPtx(int ccMajor, int ccMinor, int count)
    {
        ...
        ptx.AppendLine($"// complex-magnitude count={count}");
        ptx.AppendLine();
        ...
        ptx.AppendLine("    ld.global.nc.f32 %f0, [%rd4];");             // real
        ptx.AppendLine("    ld.global.nc.f32 %f1, [%rd5];");             // imag
        ptx.AppendLine("    mul.rn.f32 %f2, %f0, %f0;");                 // real^2
        ptx.AppendLine("    fma.rn.f32 %f2, %f1, %f1, %f2;");            // + imag^2
        ptx.AppendLine("    sqrt.rn.f32 %f3, %f2;");                     // magnitude
        ...
        return new DirectPtxKernelBlueprint(
            ...
            [
                ...
                new("imag", DirectPtxPhysicalType.Float32, DirectPtxPhysicalLayout.Vector,
                    extent, extent, 16, DirectPtxTensorAccess.Read, DirectPtxExtentMode.Exact),
                new("magnitude", DirectPtxPhysicalType.Float32, DirectPtxPhysicalLayout.Vector,
            ...
            {
                ["formula"] = "magnitude[i] = sqrt(real[i]^2 + imag[i]^2)",
                ["layout"] = "split-real-imag",
