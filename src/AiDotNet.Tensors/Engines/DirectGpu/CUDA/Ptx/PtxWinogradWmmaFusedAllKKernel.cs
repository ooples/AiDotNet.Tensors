...
/// <summary>
/// Fully-fused FP16 Tensor-Core Winograd F(2,3) 3x3 convolution that computes V
/// exactly once per tile-block. One 16-warp block owns 8 output tiles across ALL K
/// output channels: phase 1 computes V = B^T d B straight into shared (once), phase
/// 2 runs, per Winograd position (warp), K/16 mma m-subtiles reusing the shared B
/// (V) fragment across every channel-subtile while streaming the A (U) fragments,
/// and phase 3 exchanges M through shared and applies A^T M A + bias + ReLU. Unlike
/// <see cref="PtxWinogradWmmaFullyFusedKernel"/> (grid.y = K/16, which recomputed V
/// once per channel-block -> 4x redundant transform), grid.y = 1 here so V is
/// transformed a single time. The shared V region (phase 1-2) and M region (phase 3)
/// overlap because they are never live simultaneously, keeping shared at K*8*4 bytes.
/// This is the correct cuDNN-style fusion: no global V, no V round-trip, no redundant
/// transform. Specialized for K &lt;= 64 (M-exchange fits 32 KB shared).
/// </summary>
internal sealed class PtxWinogradWmmaFusedAllKKernel : IDisposable
{
    ...
    internal int OutputChannels { get; }  // K (all in one block)
    ...
    internal int MSharedBytes => 16 * OutputChannels * TileP * sizeof(float);
    internal int SharedBytes => Math.Max(VSharedBytes, MSharedBytes);   // overlap
    ...
    internal string EntryPoint => FormattableString.Invariant(
        $"aidotnet_winograd_wmma_fusedallk_n{Batch}_c{InputChannels}_h{Height}_w{Width}_k{OutputChannels}");

    internal PtxWinogradWmmaFusedAllKKernel(
        DirectPtxRuntime runtime, int batch, int inputChannels, int height, int width, int outputChannels)
    {
        ...
        if (!DirectPtxArchitecture.HasExperimentalConvolution(
            ...
            throw new NotSupportedException("Fused all-K WMMA Winograd has no experimental non-SM86 specialization.");
        ...
        DirectPtxArchitectureFamily architecture, int n, int c, int h, int w, int k)
    {
        ...
        return new DirectPtxKernelBlueprint(
            Operation: "winograd-wmma-fused-allk",
            Version: 1,
            Architecture: architecture,
            Variant: FormattableString.Invariant($"n{n}-c{c}-h{h}-w{w}-k{k}-m16n8k16-f16f32-fusedallk"),
            Tensors:
            [
            ...
            Semantics: new Dictionary<string, string>(StringComparer.Ordinal)
            {
                ...
                ["fusion"] = "input transform + all-K GEMM + output transform, V computed once (grid.y=1)",
                ["global-intermediates"] = "none",
                ["shared-reuse"] = "V region (phase1-2) overlaps M region (phase3)",
                ["mma"] = "mma.sync.aligned.m16n8k16.row.col.f32.f16.f16.f32, B(V) reused across K-subtiles",
        ...
        DirectPtxTensorView u, DirectPtxTensorView input, DirectPtxTensorView bias, DirectPtxTensorView output)
    {
        ...
        // grid: (P/8 tile-blocks, 1, 1) -- one block does all K channels; 16 warps.
        _module.Launch(
            _function,
            (uint)(Tiles / TileP), 1, 1,
            BlockThreads, 1, 1,
            0,
            args);
    }
    ...
    internal string EmitPtx(int major, int minor, int n, int c, int h, int w, int k)
    {
        if (!DirectPtxArchitecture.HasExperimentalConvolution(major, minor))
            throw new NotSupportedException("Only the experimental SM86 fused all-K WMMA Winograd emitter exists.");
        ...
        int ksteps = c / 16;
        int nsubK = k / 16;
        int vPosStride = TileP * c * sizeof(ushort);
        int vTileStride = c * sizeof(ushort);
        int items = TileP * c;
        int vBytes = 16 * TileP * c * sizeof(ushort);
        int mPosStride = k * TileP * sizeof(float);   // M_s position stride
        int accReg(int msub, int e) => msub * 4 + e;
        ...
        s.AppendLine("    .reg .f32 %c<16>;");   // nsubK <=4 subtiles x 4 accs
        s.AppendLine("    .reg .f32 %d<48>;");
        s.AppendLine("    .reg .f32 %f<32>;");
        s.AppendLine("    .reg .b32 %r<48>;");
        s.AppendLine("    .reg .b64 %rd<32>;");
        s.AppendLine($"    .shared .align 16 .b8 smem[{Math.Max(vBytes, 16 * k * TileP * 4)}];");
        ...
        // ===== PHASE 1: input transform -> V_shared (computed once) =====
        s.AppendLine("    // ---- phase 1 ----");
        s.AppendLine("    mov.u32 %r5, %r0;");
        ...
        s.AppendLine("    sub.s32 %r13, %r13, 1;");                 // ih0
        s.AppendLine("    mul.lo.s32 %r14, %r9, 2;");
        s.AppendLine("    sub.s32 %r14, %r14, 1;");                 // iw0
        s.AppendLine($"    mad.lo.u32 %r15, %r12, {I(c)}, %r7;");
        ...
        // ===== PHASE 2: all-K GEMM, B(V) reused across K-subtiles =====
        s.AppendLine("    // ---- phase 2 ----");
        ...
        // A base row g (channel 0): rd8 = u + (xi*KC + g*C + tig*2)*2 ; rd9 for row g+8
        ...
        // B base in shared V: rd10 = smem + xi*vPosStride + g*vTileStride + (tig*2)*2
        ...
        int chanRowBytes = 16 * c * sizeof(ushort);   // U rows advance 16 channels per subtile
        for (int ks = 0; ks < ksteps; ks++)
        {
            ...
            {
                int aoff = uoff + msub * chanRowBytes;
                ...
                s.AppendLine("    mma.sync.aligned.m16n8k16.row.col.f32.f16.f16.f32 " +
                    $"{{%c{accReg(msub, 0)},%c{accReg(msub, 1)},%c{accReg(msub, 2)},%c{accReg(msub, 3)}}}, " +
                    "{%a0,%a1,%a2,%a3}, {%b0,%b1}, " +
                    $"{{%c{accReg(msub, 0)},%c{accReg(msub, 1)},%c{accReg(msub, 2)},%c{accReg(msub, 3)}}};");
            }
        }
        s.AppendLine("    bar.sync 0;");   // V no longer needed; M reuses the region
        s.AppendLine();

        // ===== PHASE 3: M exchange (overlaps V) + output transform =====
        s.AppendLine("    // ---- phase 3 ----");
        // M_s[pos][ch][tile] = pos*mPosStride + ch*(TileP*4) + tile*4, at smem base.
        // this warp's subtile msub -> channels msub*16 + {g, g+8}, tiles {tig*2, tig*2+1}.
        s.AppendLine($"    mul.lo.u32 %r30, %r22, {I(mPosStride)};");
        s.AppendLine($"    mad.lo.u32 %r31, %r23, {I(TileP * 4)}, %r30;");   // + g*(TileP*4)
        s.AppendLine("    shl.b32 %r32, %r24, 3;");                          // tig*2*4
        s.AppendLine("    add.u32 %r31, %r31, %r32;");
        s.AppendLine("    cvt.u64.u32 %rd11, %r31;");
        s.AppendLine("    add.u64 %rd11, %rd4, %rd11;");
        for (int msub = 0; msub < nsubK; msub++)
        {
            int cbyte = msub * 16 * TileP * 4;   // channels advance 16 -> +16*TileP*4
            s.AppendLine($"    st.shared.f32 [%rd11+{I(cbyte)}], %c{accReg(msub, 0)};");
            s.AppendLine($"    st.shared.f32 [%rd11+{I(cbyte + 4)}], %c{accReg(msub, 1)};");
            s.AppendLine($"    st.shared.f32 [%rd11+{I(cbyte + 8 * TileP * 4)}], %c{accReg(msub, 2)};");   // +8 channels
            s.AppendLine($"    st.shared.f32 [%rd11+{I(cbyte + 8 * TileP * 4 + 4)}], %c{accReg(msub, 3)};");
        }
        s.AppendLine("    bar.sync 0;");
        // Epilogue: K*8 outputs. Loop item = tid; item < K*8; item += 512 (K*8 <= 512).
        ...
        s.AppendLine($"    mad.lo.u32 %r35, %r33, {I(TileP * 4)}, 0;"); // ch*(TileP*4)
        ...
        s.AppendLine("    mov.u32 %r37, %r33;");                     // k = ch (kbase 0)
