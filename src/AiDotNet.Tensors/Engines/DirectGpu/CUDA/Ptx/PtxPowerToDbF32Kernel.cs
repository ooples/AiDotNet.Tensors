...
internal sealed class PtxPowerToDbF32Kernel : IDisposable
{
    ...
    {
        DirectPtxAbiGuard.Require(power, Blueprint.Tensors[0], nameof(power));
        DirectPtxAbiGuard.Require(db, Blueprint.Tensors[1], nameof(db));
    ...
    internal static string EmitPtx(int ccMajor, int ccMinor, int count, int blockThreads = DefaultBlockThreads)
    {
        Validate(count);
        ValidateBlockThreads(count, blockThreads);
        string tiny = DirectPtxPtxText.Hex(1e-10f);
        string tenLog10Of2 = DirectPtxPtxText.Hex((float)(10.0 * Math.Log10(2.0)));   // 10*log10(x) = lg2(x) * (10*log10 2)
        ...
        DirectPtxArchitectureFamily architecture, int count, int blockThreads)
    {
        ...
        return new DirectPtxKernelBlueprint(
            ...
            ResourceBudget: new DirectPtxResourceBudget(
                // The logarithm expansion allocates 24 registers on SM86 and
                // still sustains the full six resident 256-thread blocks.
