...
/// <summary>
/// Exact contiguous FP32 global average pooling (issue #842):
/// <c>output[bc] = mean over the H*W spatial positions of input[bc, :]</c>, where
/// each (batch, channel) pair owns one contiguous row of <c>spatial = H*W</c>
/// elements. A warp owns one row, keeps every lane value in registers from the
/// sole global load through a butterfly-shuffle sum reduction, multiplies by the
/// baked <c>1/spatial</c> immediate, and lane zero commits one FP32 element.
/// There are no shared-memory, local-memory, global-intermediate, temporary-
/// allocation, division, remainder, stride, or scalar shape parameters; only two
/// tensor pointers reach the launch ABI. Disabled by default; fails closed until
/// three clean promotion runs clear the release gate.
/// </summary>
internal sealed class PtxFusedGlobalAvgPoolF32Kernel : IDisposable
{
    internal const string EntryPoint = "aidotnet_fused_global_avgpool_f32";
    ...
    internal PtxFusedGlobalAvgPoolF32Kernel(
        ...
        int blockThreads = DefaultBlockThreads)
    {
        ...
        Ptx = EmitPtx(runtime.ComputeCapabilityMajor, runtime.ComputeCapabilityMinor, rows, spatial, blockThreads);
    ...
    internal unsafe void Launch(DirectPtxTensorView input, DirectPtxTensorView output)
    {
        ...
        if (Overlaps(input, output))
            throw new ArgumentException("The global-average-pool specialization does not admit aliasing.");
        ...
        int blockThreads = DefaultBlockThreads)
    {
        ...
        ptx.AppendLine(
            $"// exact-shape rows={rows} spatial={spatial} block={blockThreads} strategy=warp-row op=global-avgpool cache=ca");
        ptx.AppendLine();
        ...
        ptx.AppendLine("    add.u64 %rd4, %rd1, %rd7;");
        ptx.AppendLine($"    @%p0 st.global.f32 [%rd4], %f{accReg};");
        ptx.AppendLine("    ret;");
        ptx.AppendLine("}");
        return ptx.ToString();
    }
        ...
        int blockThreads)
    {
        ...
        return new DirectPtxKernelBlueprint(
            Operation: "global-avgpool-f32",
            Version: 1,
            Architecture: architecture,
            Variant: $"warp-row-w{blockThreads / 32}-r{rows}-s{spatial}",
            Tensors:
            [
            ...
            {
                ["formula"] = "output[bc] = mean over H*W of input[bc,:]",
                ["reduction"] = "mean-over-spatial",
                ["mode"] = "inference-forward-warp-row-mean",
                ["input"] = "fp32-bchw-flattened-to-[bc,hw]",
                ["accumulator"] = "fp32-warp-reduction-register-resident",
                ["output"] = "fp32-[bc]",
                ["global-input-reads"] = "one-per-element",
                ["global-output-writes"] = "one-per-channel",
                ["lane-vector-transaction"] = spatial == 64 ? "aligned-fp32x2" : "aligned-fp32x4",
    ...
    private static void Validate(int rows, int spatial)
    {
        if (!IsSupportedShape(rows, spatial))
            throw new ArgumentOutOfRangeException(nameof(rows),
                "The first global-average-pool family supports exact (rows=batch*channels, spatial=H*W) " +
    ...
    private static void ValidateBlockThreads(int rows, int blockThreads)
    {
        if (blockThreads is not (128 or 256 or 512) ||
            ...
            throw new ArgumentOutOfRangeException(nameof(blockThreads),
                "Global-average-pool block threads must be 128, 256, or 512 and evenly tile rows.");
    }
