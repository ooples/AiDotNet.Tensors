...
/// <summary>
/// Hand-emitted FP32 NCHW deformable 3x3 convolution backward-input (DCNv2; stride 1,
/// pad 1, dilation 1, single deformable group). Formulated as a deterministic gather
/// (the transpose of the forward bilinear sampler, no atomics): the weight with which
/// input pixel (iy,ix) contributes to a sample at (py,px) is g(iy-py)*g(ix-px) with
/// g(d)=max(0,1-|d|), which exactly reproduces the four-corner bilinear coefficients and
/// vanishes elsewhere — so the input gradient needs no input tensor:
/// <c>dX[ci,iy,ix] = sum_{t,oy,ox} g(iy-py)g(ix-px) * mask[t,oy,ox] * sum_co dOut[co,oy,ox] W[co,ci,t]</c>
/// with <c>py=oy+ky-1+offY[t,oy,ox]</c>, <c>px=ox+kx-1+offX[t,oy,ox]</c>, over the exact
/// N1/Cin4/H8/W8/Cout4 geometry. One thread owns one input element and sweeps every
/// (tap, output position) via a triple loop, reducing dOut*W over co (thread-private,
/// deterministic). All tensors map to byte-exact extents.
/// </summary>
internal sealed class PtxDeformableConv2DNchw3x3BackwardInputF32Kernel : IDisposable
{
    internal const string EntryPoint = "aidotnet_deformconv2d_n1_cin4_h8_w8_cout4_r3_s1_p1_v2_bwd_input";
    ...
    internal PtxDeformableConv2DNchw3x3BackwardInputF32Kernel(DirectPtxRuntime runtime)
    {
        PtxCompat.ThrowIfNull(runtime, nameof(runtime));
        if (!DirectPtxArchitecture.HasExperimentalConvolution(
                runtime.ComputeCapabilityMajor, runtime.ComputeCapabilityMinor))
            throw new NotSupportedException(
                $"Deformable Conv2D 3x3 backward-input has no experimental SM {runtime.ComputeCapabilityMajor}.{runtime.ComputeCapabilityMinor} specialization.");
        ...
        DirectPtxArchitectureFamily architecture)
    {
        ...
        return new DirectPtxKernelBlueprint(
            Operation: "deformable-conv2d-backward-input",
            Version: 1,
            ...
            {
                ["equation"] = "dX[ci,iy,ix] = sum_{t,oy,ox} g(iy-py)g(ix-px) * mask[t,oy,ox] * sum_co dOut[co,oy,ox] W[co,ci,t]; g(d)=max(0,1-|d|)",
                ["grad-output"] = "fp32",
                ...
                ["deformable-groups"] = "1",
                ["layout"] = "nchw grad-output/grad-input; oihw weights; [2*kHkW,outH,outW] offsets; [kHkW,outH,outW] mask",
    ...
    internal static string EmitPtx(int computeCapabilityMajor, int computeCapabilityMinor)
    {
        if (!DirectPtxArchitecture.HasExperimentalConvolution(
            ...
            throw new NotSupportedException(
                "Only the experimental SM86 deformable Conv2D 3x3 backward-input emitter exists.");
        ...
        const int ciWeightBaseBytes = TapsPerChannel * sizeof(float);                         // ci*36 (Cin col)
        ...
        // Weight column base for ci: weights + ci*36.
        ...
        ptx.AppendLine("    mov.u32 %r7, 0;");                                 // tap counter t
        ptx.AppendLine("DEFORM_BWD_INPUT_T:");
        ptx.AppendLine("    div.u32 %r8, %r7, 3;");                            // ky
        ptx.AppendLine("    mul.lo.u32 %r9, %r8, 3;");
        ptx.AppendLine("    sub.u32 %r10, %r7, %r9;");                         // kx
        ...
        ptx.AppendLine("DEFORM_BWD_INPUT_S:");
        ptx.AppendLine("    shr.u32 %r14, %r13, 3;");                          // oy
        ptx.AppendLine($"    and.b32 %r15, %r13, {Width - 1};");               // ox
        // offsets: offY at (t*128 + s)*4, offX at +256.
        ptx.AppendLine("    mad.lo.u32 %r16, %r7, 128, %r13;");
        ptx.AppendLine("    mul.wide.u32 %rd7, %r16, 4;");
        ...
        // mask at (t*64 + s)*4.
        ptx.AppendLine("    mad.lo.u32 %r17, %r7, 64, %r13;");
        ptx.AppendLine("    mul.wide.u32 %rd8, %r17, 4;");
        ptx.AppendLine("    add.u64 %rd8, %rd3, %rd8;");
        ptx.AppendLine("    ld.global.nc.f32 %f3, [%rd8];");                   // mask_ts
        // py, px.
        ptx.AppendLine("    add.s32 %r18, %r14, %r11;");                        // oy + ky - 1
        ptx.AppendLine("    cvt.rn.f32.s32 %f4, %r18;");
        ptx.AppendLine("    add.rn.f32 %f4, %f4, %f1;");                        // py
        ptx.AppendLine("    add.s32 %r19, %r15, %r12;");                        // ox + kx - 1
        ptx.AppendLine("    cvt.rn.f32.s32 %f5, %r19;");
        ptx.AppendLine("    add.rn.f32 %f5, %f5, %f2;");                        // px
        // gy = max(0, 1 - |iy - py|), gx = max(0, 1 - |ix - px|).
        ptx.AppendLine("    sub.rn.f32 %f6, %f13, %f4;");                       // iy - py
        ptx.AppendLine("    abs.f32 %f6, %f6;");
        ptx.AppendLine("    sub.rn.f32 %f6, %f15, %f6;");                       // 1 - |dy|
        ptx.AppendLine("    max.f32 %f6, %f6, 0f00000000;");                    // gy
        ptx.AppendLine("    sub.rn.f32 %f7, %f14, %f5;");                       // ix - px
        ptx.AppendLine("    abs.f32 %f7, %f7;");
        ptx.AppendLine("    sub.rn.f32 %f7, %f15, %f7;");                       // 1 - |dx|
        ...
        ptx.AppendLine("    add.u64 %rd9, %rd0, %rd9;");                        // dOut + s*4 (dco, +256 per co)
        ptx.AppendLine("    mov.u64 %rd10, %rd6;");                            // wco = wtap (+144 per co)
        ptx.AppendLine("    mov.f32 %f10, 0f00000000;");                       // G
        ptx.AppendLine("    mov.u32 %r20, 0;");                                // co counter
        ptx.AppendLine("DEFORM_BWD_INPUT_CO:");
        ...
        ptx.AppendLine("    add.u32 %r20, %r20, 1;");
        ptx.AppendLine($"    setp.lt.u32 %p0, %r20, {OutputChannels};");
        ptx.AppendLine("    @%p0 bra DEFORM_BWD_INPUT_CO;");
        ptx.AppendLine("    fma.rn.f32 %f0, %f9, %f10, %f0;");                  // acc += cm * G
        ptx.AppendLine("    add.u32 %r13, %r13, 1;");
        ptx.AppendLine($"    setp.lt.u32 %p1, %r13, {SpatialElements};");
        ptx.AppendLine("    @%p1 bra DEFORM_BWD_INPUT_S;");
        ptx.AppendLine("    add.u32 %r7, %r7, 1;");
        ptx.AppendLine($"    setp.lt.u32 %p2, %r7, {TapsPerChannel};");
        ptx.AppendLine("    @%p2 bra DEFORM_BWD_INPUT_T;");
