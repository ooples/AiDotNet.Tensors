...
/// <summary>
/// Hand-emitted FP32 NCDHW 3x3x3 3D convolution with fused bias and ReLU (stride 1,
/// zero pad 1):
/// <c>out[co,d,h,w] = relu(bias[co] + sum_ci sum_{kd,kh,kw} W[co,ci,kd,kh,kw] * in[ci, d+kd-1, h+kh-1, w+kw-1])</c>
/// over the exact N1/Cin16/D8/H8/W8/Cout16 geometry, OIDHW weights. Structurally
/// identical to <see cref="PtxConv3DNcdhw3x3x3ForwardF32Kernel"/> plus a five-instruction
/// epilogue that seeds nothing and instead folds the bias add and activation into the
/// accumulator's final register, so no intermediate 3D convolution result is written to
/// global memory. NCDHW/OIDHW logical shapes are rank-5, so the ABI contract uses
/// byte-exact collapsed rank-4 extents; the emitter is the authority on the true strides.
/// </summary>
internal sealed class PtxFusedConv3DNcdhw3x3x3BiasReluF32Kernel : IDisposable
{
    internal const string EntryPoint = "aidotnet_conv3d_n1_cin16_d8_h8_w8_cout16_r3_s1_p1_bias_relu";
    ...
    internal PtxFusedConv3DNcdhw3x3x3BiasReluF32Kernel(DirectPtxRuntime runtime)
    {
        PtxCompat.ThrowIfNull(runtime, nameof(runtime));
        if (!DirectPtxArchitecture.HasExperimentalConvolution(
                runtime.ComputeCapabilityMajor, runtime.ComputeCapabilityMinor))
            throw new NotSupportedException(
                $"Fused Conv3D 3x3x3 has no experimental SM {runtime.ComputeCapabilityMajor}.{runtime.ComputeCapabilityMinor} specialization.");
        ...
        DirectPtxArchitectureFamily architecture)
    {
        ...
        return new DirectPtxKernelBlueprint(
            ...
            {
                ["equation"] = "relu(bias[co] + sum_ci sum_{kd,kh,kw} W[co,ci,kd,kh,kw] * in[ci, d+kd-1, h+kh-1, w+kw-1])",
                ["input"] = "fp32",
    ...
    internal static string EmitPtx(int computeCapabilityMajor, int computeCapabilityMinor)
    {
        if (!DirectPtxArchitecture.HasExperimentalConvolution(
            ...
            throw new NotSupportedException(
                "Only the experimental SM86 fused Conv3D 3x3x3 emitter exists.");

        const int inChannelStrideBytes = SpatialElements * sizeof(float);                 // per ci  (2048)
        const int weightChannelStrideBytes = TapsPerChannel * sizeof(float);              // per (co,ci)  (108)
        const int weightOutStrideBytes = InputChannels * TapsPerChannel * sizeof(float);  // per co  (1728)
        ...
        ptx.AppendLine($"    mul.wide.u32 %rd4, %r7, {weightOutStrideBytes};");
        ...
        ptx.AppendLine("CONV3D_FUSED_CHANNELS:");
        for (int m = 0; m < TapsPerChannel; m++)
        {
            ...
            if (guards.Count == 0)
            {
                ptx.AppendLine("    ld.global.nc.f32 %f1, [%rd5];");           // center tap (always valid)
            }
            else
            {
                string guard;
        ...
        ptx.AppendLine($"    add.u64 %rd4, %rd4, {weightChannelStrideBytes};");
        ...
        ptx.AppendLine("    @%p7 bra CONV3D_FUSED_CHANNELS;");
