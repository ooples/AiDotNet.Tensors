...
// CUDA kernels for ConvLSTM (Convolutional LSTM) neural network operations.
// Implements spatiotemporal sequence learning with convolutional gates.

namespace AiDotNet.Tensors.Engines.DirectGpu.CUDA.Kernels;

/// <summary>
/// CUDA kernels for ConvLSTM operations used in spatiotemporal neural networks.
/// Implements full forward and backward passes for ConvLSTM cells with 4 gates.
/// </summary>
internal static class CudaConvLSTMKernels
{
...
// ConvLSTM cell forward pass for a single time step
// Computes all 4 gates (forget, input, cell, output) and new states
// input: [batch, inputChannels, height, width]
// prevH: [batch, hiddenChannels, height, width]
// prevC: [batch, hiddenChannels, height, width]
// weights*i: [hiddenChannels, inputChannels, kernelH, kernelW]
    ...
    int height, int width, int kernelH, int kernelW, int pad)
{
    ...
    // Compute convolutions for all 4 gates
    float sumF = biasF[hc];
    float sumI = biasI[hc];
    float sumC = biasC[hc];
    float sumO = biasO[hc];

    // Input convolutions (input * weights_i)
    ...
    // Hidden state convolutions (prevH * weights_h)
    ...
    // Compute new cell state: newC = f * prevC + i * c
    ...
    int height, int width, int kernelH, int kernelW, int pad)
{
    ...
    // Backprop through all hidden channels
    ...
    int batch, int hiddenChannels, int height, int width)
{
    ...
    // Gradient from hidden state through cell state
    float dH = gradH[gid];
    float dC_from_H = dH * o * tanh_derivative(tanhNewC);
    ...
    // Gradient to previous cell state: dPrevC = dC * f
    gradPrevC[gid] = dC * f;
}
    ...
    int height, int width, int kernelH, int kernelW, int pad)
{
    ...
    // Accumulate gradients over batch and spatial dimensions
