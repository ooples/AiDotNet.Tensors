...
";
    ...
    #region Fused Kernel Dispatch Helpers

    private unsafe void LaunchFusedUnary(string kn, IGpuBuffer i, IGpuBuffer o, int sz) { if (!_kernelCache.TryGetValue(kn, out var k)) throw new InvalidOperationException($"CUDA kernel not found: {kn}"); using var _ = PushContext(); IntPtr ip = i.Handle, op = o.Handle; void** a = stackalloc void*[3]; a[0]=&ip;a[1]=&op;a[2]=&sz; LaunchKernel(k,(uint)((sz+DefaultBlockSize-1)/DefaultBlockSize),DefaultBlockSize,a); }
    private unsafe void LaunchFusedAxis(string kn, IGpuBuffer i, IGpuBuffer o, int os, int rs) { if (!_kernelCache.TryGetValue(kn, out var k)) throw new InvalidOperationException($"CUDA kernel not found: {kn}"); using var _ = PushContext(); IntPtr ip=i.Handle,op=o.Handle; void** a=stackalloc void*[4]; a[0]=&ip;a[1]=&op;a[2]=&os;a[3]=&rs; LaunchKernel(k,(uint)((os+DefaultBlockSize-1)/DefaultBlockSize),DefaultBlockSize,a); }
    private unsafe void LaunchFusedScalar(string kn, IGpuBuffer i, IGpuBuffer o, float sc, int sz) { if (!_kernelCache.TryGetValue(kn, out var k)) throw new InvalidOperationException($"CUDA kernel not found: {kn}"); using var _ = PushContext(); IntPtr ip=i.Handle,op=o.Handle; void** a=stackalloc void*[4]; a[0]=&ip;a[1]=&op;a[2]=&sc;a[3]=&sz; LaunchKernel(k,(uint)((sz+DefaultBlockSize-1)/DefaultBlockSize),DefaultBlockSize,a); }
    private unsafe void LaunchFusedBinary(string kn, IGpuBuffer i1, IGpuBuffer i2, IGpuBuffer o, int sz) { if (!_kernelCache.TryGetValue(kn, out var k)) throw new InvalidOperationException($"CUDA kernel not found: {kn}"); using var _ = PushContext(); IntPtr p1=i1.Handle,p2=i2.Handle,op=o.Handle; void** a=stackalloc void*[4]; a[0]=&p1;a[1]=&p2;a[2]=&op;a[3]=&sz; LaunchKernel(k,(uint)((sz+DefaultBlockSize-1)/DefaultBlockSize),DefaultBlockSize,a); }
    private unsafe void LaunchBroadcast(string kn, IGpuBuffer a1, IGpuBuffer b1, IGpuBuffer o, int os, int isz) { if (!_kernelCache.TryGetValue(kn, out var k)) throw new InvalidOperationException($"CUDA kernel not found: {kn}"); using var _ = PushContext(); IntPtr ap=a1.Handle,bp=b1.Handle,op=o.Handle; void** a=stackalloc void*[5]; a[0]=&ap;a[1]=&bp;a[2]=&op;a[3]=&os;a[4]=&isz; uint t=(uint)(os*isz); LaunchKernel(k,(t+DefaultBlockSize-1)/DefaultBlockSize,DefaultBlockSize,a); }
    private unsafe void LaunchGatedBackward(string kn, IGpuBuffer go, IGpuBuffer inp, IGpuBuffer gi, int os, int hd) { if (!_kernelCache.TryGetValue(kn, out var k)) throw new InvalidOperationException($"CUDA kernel not found: {kn}"); using var _ = PushContext(); IntPtr gop=go.Handle,ip=inp.Handle,gip=gi.Handle; void** a=stackalloc void*[5]; a[0]=&gop;a[1]=&ip;a[2]=&gip;a[3]=&os;a[4]=&hd; uint t=(uint)(os*hd); LaunchKernel(k,(t+DefaultBlockSize-1)/DefaultBlockSize,DefaultBlockSize,a); }

    #endregion

    #region Fused Kernel Dispatch (84 IDirectGpuBackend methods)

    public void ReduceMean(IGpuBuffer i, IGpuBuffer o, int sz) => LaunchFusedUnary("reduce_mean",i,o,sz);
    public void ReduceProduct(IGpuBuffer i, IGpuBuffer o, int sz) => LaunchFusedUnary("reduce_product",i,o,sz);
    public void ReduceNormL2(IGpuBuffer i, IGpuBuffer o, int sz) => LaunchFusedUnary("reduce_norm_l2",i,o,sz);
    public void ReduceSumOfSquares(IGpuBuffer i, IGpuBuffer o, int sz) => LaunchFusedUnary("reduce_sum_of_squares",i,o,sz);
    public void ReduceMaxMagnitude(IGpuBuffer i, IGpuBuffer o, int sz) => LaunchFusedUnary("reduce_max_magnitude",i,o,sz);
    public void ReduceMinMagnitude(IGpuBuffer i, IGpuBuffer o, int sz) => LaunchFusedUnary("reduce_min_magnitude",i,o,sz);
    public unsafe void ReduceLogSumExp(IGpuBuffer i, IGpuBuffer o, float mx, int sz) { if (!_kernelCache.TryGetValue("reduce_logsumexp",out var k)) throw new InvalidOperationException("CUDA kernel not found: reduce_logsumexp"); using var _=PushContext(); IntPtr ip=i.Handle,op=o.Handle; void** a=stackalloc void*[4]; a[0]=&ip;a[1]=&op;a[2]=&mx;a[3]=&sz; LaunchKernel(k,(uint)((sz+DefaultBlockSize-1)/DefaultBlockSize),DefaultBlockSize,a); }
    public void VarianceAxis(IGpuBuffer i, IGpuBuffer o, int os, int rs) => LaunchFusedAxis("variance_axis",i,o,os,rs);
    public void StdAxis(IGpuBuffer i, IGpuBuffer o, int os, int rs) => LaunchFusedAxis("std_axis",i,o,os,rs);
    public void ProductAxis(IGpuBuffer i, IGpuBuffer o, int os, int rs) => LaunchFusedAxis("product_axis",i,o,os,rs);
    public void NormAxis(IGpuBuffer i, IGpuBuffer o, int os, int rs) => LaunchFusedAxis("norm_axis",i,o,os,rs);
    public void LogSumExpAxis(IGpuBuffer i, IGpuBuffer o, int os, int rs) => LaunchFusedAxis("logsumexp_axis",i,o,os,rs);
    public void CumSumAxis(IGpuBuffer i, IGpuBuffer o, int os, int isz) => LaunchFusedAxis("cumsum_axis",i,o,os,isz);
    public void ScalarMinusTensor(IGpuBuffer i, IGpuBuffer o, float sc, int sz) => LaunchFusedScalar("scalar_minus_tensor",i,o,sc,sz);
    public void NormalizeL2(IGpuBuffer i, IGpuBuffer o, int os, int isz) => LaunchFusedAxis("normalize_l2",i,o,os,isz);
    public void ReduceSumBackward(IGpuBuffer go, IGpuBuffer gi, int os, int rs) => LaunchFusedAxis("reduce_sum_backward",go,gi,os,rs);
    public void ReduceMeanBackward(IGpuBuffer go, IGpuBuffer gi, int os, int rs) => LaunchFusedAxis("reduce_mean_backward",go,gi,os,rs);
    public unsafe void ReduceMaxBackward(IGpuBuffer go, IGpuBuffer inp, IGpuBuffer mx, IGpuBuffer gi, int os, int rs) { if (!_kernelCache.TryGetValue("reduce_max_backward",out var k)) throw new InvalidOperationException("CUDA kernel not found: reduce_max_backward"); using var _=PushContext(); IntPtr gop=go.Handle,ip=inp.Handle,mp=mx.Handle,gip=gi.Handle; void** a=stackalloc void*[6]; a[0]=&gop;a[1]=&ip;a[2]=&mp;a[3]=&gip;a[4]=&os;a[5]=&rs; LaunchKernel(k,(uint)((os*rs+DefaultBlockSize-1)/DefaultBlockSize),DefaultBlockSize,a); }
    public unsafe void ReduceVarianceBackward(IGpuBuffer go, IGpuBuffer inp, IGpuBuffer ms, IGpuBuffer gi, int os, int rs) { if (!_kernelCache.TryGetValue("reduce_variance_backward",out var k)) throw new InvalidOperationException("CUDA kernel not found: reduce_variance_backward"); using var _=PushContext(); IntPtr gop=go.Handle,ip=inp.Handle,mp=ms.Handle,gip=gi.Handle; void** a=stackalloc void*[6]; a[0]=&gop;a[1]=&ip;a[2]=&mp;a[3]=&gip;a[4]=&os;a[5]=&rs; LaunchKernel(k,(uint)((os*rs+DefaultBlockSize-1)/DefaultBlockSize),DefaultBlockSize,a); }
    public void ReduceLogVariance(IGpuBuffer i, IGpuBuffer o, int os, int rs) => LaunchFusedAxis("reduce_log_variance",i,o,os,rs);
    public unsafe void ReduceLogVarianceBackward(IGpuBuffer go, IGpuBuffer inp, IGpuBuffer ms, IGpuBuffer vs, IGpuBuffer gi, int os, int rs) { if (!_kernelCache.TryGetValue("reduce_log_variance_backward",out var k)) throw new InvalidOperationException("CUDA kernel not found: reduce_log_variance_backward"); using var _=PushContext(); IntPtr gop=go.Handle,ip=inp.Handle,mp=ms.Handle,vp=vs.Handle,gip=gi.Handle; void** a=stackalloc void*[7]; a[0]=&gop;a[1]=&ip;a[2]=&mp;a[3]=&vp;a[4]=&gip;a[5]=&os;a[6]=&rs; LaunchKernel(k,(uint)((os*rs+DefaultBlockSize-1)/DefaultBlockSize),DefaultBlockSize,a); }

    public void BroadcastAddLast(IGpuBuffer a1, IGpuBuffer b1, IGpuBuffer o, int os, int isz) => LaunchBroadcast("broadcast_add_last",a1,b1,o,os,isz);
    public void BroadcastSubLast(IGpuBuffer a1, IGpuBuffer b1, IGpuBuffer o, int os, int isz) => LaunchBroadcast("broadcast_sub_last",a1,b1,o,os,isz);
    public void BroadcastMulLast(IGpuBuffer a1, IGpuBuffer b1, IGpuBuffer o, int os, int isz) => LaunchBroadcast("broadcast_mul_last",a1,b1,o,os,isz);
    public void BroadcastDivLast(IGpuBuffer a1, IGpuBuffer b1, IGpuBuffer o, int os, int isz) => LaunchBroadcast("broadcast_div_last",a1,b1,o,os,isz);
    public void BroadcastAddFirst(IGpuBuffer a1, IGpuBuffer b1, IGpuBuffer o, int os, int isz) => LaunchBroadcast("broadcast_add_first",a1,b1,o,os,isz);
    public void BroadcastMulFirst(IGpuBuffer a1, IGpuBuffer b1, IGpuBuffer o, int os, int isz) => LaunchBroadcast("broadcast_mul_first",a1,b1,o,os,isz);
    public void AddScalar(IGpuBuffer i, IGpuBuffer o, float sc, int sz) => LaunchFusedScalar("add_scalar",i,o,sc,sz);
    public void SubScalar(IGpuBuffer i, IGpuBuffer o, float sc, int sz) => LaunchFusedScalar("sub_scalar",i,o,sc,sz);
    public void DivScalar(IGpuBuffer i, IGpuBuffer o, float sc, int sz) => LaunchFusedScalar("div_scalar",i,o,sc,sz);
    public void PowScalar(IGpuBuffer i, IGpuBuffer o, float ex, int sz) => LaunchFusedScalar("pow_scalar",i,o,ex,sz);
    public void FracKernel(IGpuBuffer i, IGpuBuffer o, int sz) => LaunchFusedUnary("frac_kernel",i,o,sz);
    public unsafe void ClipKernel(IGpuBuffer i, IGpuBuffer o, float mn, float mx, int sz) { if (!_kernelCache.TryGetValue("clip_kernel",out var k)) throw new InvalidOperationException("CUDA kernel not found: clip_kernel"); using var _=PushContext(); IntPtr ip=i.Handle,op=o.Handle; void** a=stackalloc void*[5]; a[0]=&ip;a[1]=&op;a[2]=&mn;a[3]=&mx;a[4]=&sz; LaunchKernel(k,(uint)((sz+DefaultBlockSize-1)/DefaultBlockSize),DefaultBlockSize,a); }
    public void RsqrtKernel(IGpuBuffer i, IGpuBuffer o, int sz) => LaunchFusedUnary("rsqrt_kernel",i,o,sz);
    public unsafe void SinCosKernel(IGpuBuffer i, IGpuBuffer so, IGpuBuffer co, int sz) { if (!_kernelCache.TryGetValue("sincos_kernel",out var k)) throw new InvalidOperationException("CUDA kernel not found: sincos_kernel"); using var _=PushContext(); IntPtr ip=i.Handle,sp=so.Handle,cp=co.Handle; void** a=stackalloc void*[4]; a[0]=&ip;a[1]=&sp;a[2]=&cp;a[3]=&sz; LaunchKernel(k,(uint)((sz+DefaultBlockSize-1)/DefaultBlockSize),DefaultBlockSize,a); }
    public void EqualsKernel(IGpuBuffer a1, IGpuBuffer b1, IGpuBuffer o, int sz) => LaunchFusedBinary("equals_kernel",a1,b1,o,sz);
    public void NotEqualsKernel(IGpuBuffer a1, IGpuBuffer b1, IGpuBuffer o, int sz) => LaunchFusedBinary("not_equals_kernel",a1,b1,o,sz);

    public void GluForward(IGpuBuffer i, IGpuBuffer o, int os, int hd) => LaunchFusedAxis("glu_forward",i,o,os,hd);
    public void GluBackward(IGpuBuffer go, IGpuBuffer inp, IGpuBuffer gi, int os, int hd) => LaunchGatedBackward("glu_backward",go,inp,gi,os,hd);
    public void GeGluForward(IGpuBuffer i, IGpuBuffer o, int os, int hd) => LaunchFusedAxis("geglu_forward",i,o,os,hd);
    public void GeGluBackward(IGpuBuffer go, IGpuBuffer inp, IGpuBuffer gi, int os, int hd) => LaunchGatedBackward("geglu_backward",go,inp,gi,os,hd);
    public void ReGluForward(IGpuBuffer i, IGpuBuffer o, int os, int hd) => LaunchFusedAxis("reglu_forward",i,o,os,hd);
    public void ReGluBackward(IGpuBuffer go, IGpuBuffer inp, IGpuBuffer gi, int os, int hd) => LaunchGatedBackward("reglu_backward",go,inp,gi,os,hd);
    public void SwiGluForward(IGpuBuffer i, IGpuBuffer o, int os, int hd) => LaunchFusedAxis("swiglu_forward",i,o,os,hd);
    public void SwiGluBackward(IGpuBuffer go, IGpuBuffer inp, IGpuBuffer gi, int os, int hd) => LaunchGatedBackward("swiglu_backward",go,inp,gi,os,hd);
    public void ReluDerivative(IGpuBuffer i, IGpuBuffer o, int sz) => LaunchFusedUnary("relu_derivative",i,o,sz);
    public void SigmoidDerivative(IGpuBuffer i, IGpuBuffer o, int sz) => LaunchFusedUnary("sigmoid_derivative",i,o,sz);
    public void TanhDerivative(IGpuBuffer i, IGpuBuffer o, int sz) => LaunchFusedUnary("tanh_derivative",i,o,sz);

    public unsafe void ConcatAxis(IGpuBuffer a1, IGpuBuffer b1, IGpuBuffer o, int os, int ais, int bis) { if (!_kernelCache.TryGetValue("concat_axis",out var k)) throw new InvalidOperationException("CUDA kernel not found: concat_axis"); using var _=PushContext(); IntPtr ap=a1.Handle,bp=b1.Handle,op=o.Handle; void** a=stackalloc void*[6]; a[0]=&ap;a[1]=&bp;a[2]=&op;a[3]=&os;a[4]=&ais;a[5]=&bis; uint t=(uint)(os*(ais+bis)); LaunchKernel(k,(t+DefaultBlockSize-1)/DefaultBlockSize,DefaultBlockSize,a); }
    public unsafe void SliceLastAxis(IGpuBuffer i, IGpuBuffer o, int os, int iis, int st, int ss) { if (!_kernelCache.TryGetValue("slice_last_axis",out var k)) throw new InvalidOperationException("CUDA kernel not found: slice_last_axis"); using var _=PushContext(); IntPtr ip=i.Handle,op=o.Handle; void** a=stackalloc void*[6]; a[0]=&ip;a[1]=&op;a[2]=&os;a[3]=&iis;a[4]=&st;a[5]=&ss; uint t=(uint)(os*ss); LaunchKernel(k,(t+DefaultBlockSize-1)/DefaultBlockSize,DefaultBlockSize,a); }
    public unsafe void SetSliceLastAxis(IGpuBuffer o, IGpuBuffer v, int os, int ois, int st, int ss) { if (!_kernelCache.TryGetValue("set_slice_last_axis",out var k)) throw new InvalidOperationException("CUDA kernel not found: set_slice_last_axis"); using var _=PushContext(); IntPtr op=o.Handle,vp=v.Handle; void** a=stackalloc void*[6]; a[0]=&op;a[1]=&vp;a[2]=&os;a[3]=&ois;a[4]=&st;a[5]=&ss; uint t=(uint)(os*ss); LaunchKernel(k,(t+DefaultBlockSize-1)/DefaultBlockSize,DefaultBlockSize,a); }
    public void Stack2(IGpuBuffer a1, IGpuBuffer b1, IGpuBuffer o, int sz) => LaunchFusedBinary("stack_2",a1,b1,o,sz);
    public unsafe void Pad2D(IGpuBuffer i, IGpuBuffer o, int ba, int ch, int ih, int iw, int oh, int ow, int pt, int pl, float pv) { if (!_kernelCache.TryGetValue("pad_2d",out var k)) throw new InvalidOperationException("CUDA kernel not found: pad_2d"); using var _=PushContext(); IntPtr ip=i.Handle,op=o.Handle; void** a=stackalloc void*[11]; a[0]=&ip;a[1]=&op;a[2]=&ba;a[3]=&ch;a[4]=&ih;a[5]=&iw;a[6]=&oh;a[7]=&ow;a[8]=&pt;a[9]=&pl;a[10]=&pv; uint t=(uint)(ba*ch*oh*ow); LaunchKernel(k,(t+DefaultBlockSize-1)/DefaultBlockSize,DefaultBlockSize,a); }
    public unsafe void Pad2DBackward(IGpuBuffer go, IGpuBuffer gi, int ba, int ch, int ih, int iw, int oh, int ow, int pt, int pl) { if (!_kernelCache.TryGetValue("pad_2d_backward",out var k)) throw new InvalidOperationException("CUDA kernel not found: pad_2d_backward"); using var _=PushContext(); IntPtr gop=go.Handle,gip=gi.Handle; void** a=stackalloc void*[10]; a[0]=&gop;a[1]=&gip;a[2]=&ba;a[3]=&ch;a[4]=&ih;a[5]=&iw;a[6]=&oh;a[7]=&ow;a[8]=&pt;a[9]=&pl; uint t=(uint)(ba*ch*ih*iw); LaunchKernel(k,(t+DefaultBlockSize-1)/DefaultBlockSize,DefaultBlockSize,a); }
    public unsafe void TileLastAxis(IGpuBuffer i, IGpuBuffer o, int os, int isz, int rp) { if (!_kernelCache.TryGetValue("tile_last_axis",out var k)) throw new InvalidOperationException("CUDA kernel not found: tile_last_axis"); using var _=PushContext(); IntPtr ip=i.Handle,op=o.Handle; void** a=stackalloc void*[5]; a[0]=&ip;a[1]=&op;a[2]=&os;a[3]=&isz;a[4]=&rp; uint t=(uint)(os*isz*rp); LaunchKernel(k,(t+DefaultBlockSize-1)/DefaultBlockSize,DefaultBlockSize,a); }
    public unsafe void RepeatElements(IGpuBuffer i, IGpuBuffer o, int os, int isz, int rp) { if (!_kernelCache.TryGetValue("repeat_elements",out var k)) throw new InvalidOperationException("CUDA kernel not found: repeat_elements"); using var _=PushContext(); IntPtr ip=i.Handle,op=o.Handle; void** a=stackalloc void*[5]; a[0]=&ip;a[1]=&op;a[2]=&os;a[3]=&isz;a[4]=&rp; uint t=(uint)(os*isz*rp); LaunchKernel(k,(t+DefaultBlockSize-1)/DefaultBlockSize,DefaultBlockSize,a); }
    public unsafe void PixelShuffle(IGpuBuffer i, IGpuBuffer o, int ba, int ch, int ih, int iw, int sc) { if (!_kernelCache.TryGetValue("pixel_shuffle",out var k)) throw new InvalidOperationException("CUDA kernel not found: pixel_shuffle"); using var _=PushContext(); IntPtr ip=i.Handle,op=o.Handle; void** a=stackalloc void*[7]; a[0]=&ip;a[1]=&op;a[2]=&ba;a[3]=&ch;a[4]=&ih;a[5]=&iw;a[6]=&sc; uint t=(uint)(ba*ch*ih*sc*iw*sc); LaunchKernel(k,(t+DefaultBlockSize-1)/DefaultBlockSize,DefaultBlockSize,a); }
    public unsafe void PixelShuffleBackward(IGpuBuffer go, IGpuBuffer gi, int ba, int ch, int ih, int iw, int sc) { if (!_kernelCache.TryGetValue("pixel_shuffle_backward",out var k)) throw new InvalidOperationException("CUDA kernel not found: pixel_shuffle_backward"); using var _=PushContext(); IntPtr gop=go.Handle,gip=gi.Handle; void** a=stackalloc void*[7]; a[0]=&gop;a[1]=&gip;a[2]=&ba;a[3]=&ch;a[4]=&ih;a[5]=&iw;a[6]=&sc; uint t=(uint)(ba*ch*sc*sc*ih*iw); LaunchKernel(k,(t+DefaultBlockSize-1)/DefaultBlockSize,DefaultBlockSize,a); }
    public unsafe void Crop2D(IGpuBuffer i, IGpuBuffer o, int ba, int ch, int ih, int iw, int oh, int ow, int ofh, int ofw) { if (!_kernelCache.TryGetValue("crop_2d",out var k)) throw new InvalidOperationException("CUDA kernel not found: crop_2d"); using var _=PushContext(); IntPtr ip=i.Handle,op=o.Handle; void** a=stackalloc void*[10]; a[0]=&ip;a[1]=&op;a[2]=&ba;a[3]=&ch;a[4]=&ih;a[5]=&iw;a[6]=&oh;a[7]=&ow;a[8]=&ofh;a[9]=&ofw; uint t=(uint)(ba*ch*oh*ow); LaunchKernel(k,(t+DefaultBlockSize-1)/DefaultBlockSize,DefaultBlockSize,a); }
    public unsafe void Crop2DBackward(IGpuBuffer go, IGpuBuffer gi, int ba, int ch, int ih, int iw, int oh, int ow, int ofh, int ofw) { if (!_kernelCache.TryGetValue("crop_2d_backward",out var k)) throw new InvalidOperationException("CUDA kernel not found: crop_2d_backward"); using var _=PushContext(); IntPtr gop=go.Handle,gip=gi.Handle; void** a=stackalloc void*[10]; a[0]=&gop;a[1]=&gip;a[2]=&ba;a[3]=&ch;a[4]=&ih;a[5]=&iw;a[6]=&oh;a[7]=&ow;a[8]=&ofh;a[9]=&ofw; uint t=(uint)(ba*ch*oh*ow); LaunchKernel(k,(t+DefaultBlockSize-1)/DefaultBlockSize,DefaultBlockSize,a); }
    public unsafe void EyeKernel(IGpuBuffer o, int n) { if (!_kernelCache.TryGetValue("eye_kernel",out var k)) throw new InvalidOperationException("CUDA kernel not found: eye_kernel"); using var _=PushContext(); IntPtr op=o.Handle; void** a=stackalloc void*[2]; a[0]=&op;a[1]=&n; LaunchKernel(k,(uint)((n*n+DefaultBlockSize-1)/DefaultBlockSize),DefaultBlockSize,a); }
    public unsafe void LinspaceKernel(IGpuBuffer o, float st, float sp, int sz) { if (!_kernelCache.TryGetValue("linspace_kernel",out var k)) throw new InvalidOperationException("CUDA kernel not found: linspace_kernel"); using var _=PushContext(); IntPtr op=o.Handle; void** a=stackalloc void*[4]; a[0]=&op;a[1]=&st;a[2]=&sp;a[3]=&sz; LaunchKernel(k,(uint)((sz+DefaultBlockSize-1)/DefaultBlockSize),DefaultBlockSize,a); }
    public unsafe void OneHotKernel(IGpuBuffer idx, IGpuBuffer o, int bs, int nc) { if (!_kernelCache.TryGetValue("one_hot_kernel",out var k)) throw new InvalidOperationException("CUDA kernel not found: one_hot_kernel"); using var _=PushContext(); IntPtr ip=idx.Handle,op=o.Handle; void** a=stackalloc void*[4]; a[0]=&ip;a[1]=&op;a[2]=&bs;a[3]=&nc; uint t=(uint)(bs*nc); LaunchKernel(k,(t+DefaultBlockSize-1)/DefaultBlockSize,DefaultBlockSize,a); }
    public unsafe void DiagKernel(IGpuBuffer i, IGpuBuffer o, int n) { if (!_kernelCache.TryGetValue("diag_kernel",out var k)) throw new InvalidOperationException("CUDA kernel not found: diag_kernel"); using var _=PushContext(); IntPtr ip=i.Handle,op=o.Handle; void** a=stackalloc void*[3]; a[0]=&ip;a[1]=&op;a[2]=&n; LaunchKernel(k,(uint)((n*n+DefaultBlockSize-1)/DefaultBlockSize),DefaultBlockSize,a); }
    public unsafe void ExtractDiagKernel(IGpuBuffer i, IGpuBuffer o, int n, int cols) { if (!_kernelCache.TryGetValue("extract_diag_kernel",out var k)) throw new InvalidOperationException("CUDA kernel not found: extract_diag_kernel"); using var _=PushContext(); IntPtr ip=i.Handle,op=o.Handle; void** a=stackalloc void*[4]; a[0]=&ip;a[1]=&op;a[2]=&n;a[3]=&cols; LaunchKernel(k,(uint)((n+DefaultBlockSize-1)/DefaultBlockSize),DefaultBlockSize,a); }
    public unsafe void TriangularMask(IGpuBuffer o, int rows, int cols, int diag, float mv) { if (!_kernelCache.TryGetValue("triangular_mask",out var k)) throw new InvalidOperationException("CUDA kernel not found: triangular_mask"); using var _=PushContext(); IntPtr op=o.Handle; void** a=stackalloc void*[5]; a[0]=&op;a[1]=&rows;a[2]=&cols;a[3]=&diag;a[4]=&mv; uint t=(uint)(rows*cols); LaunchKernel(k,(t+DefaultBlockSize-1)/DefaultBlockSize,DefaultBlockSize,a); }
    public unsafe void MaskedFillKernel(IGpuBuffer i, IGpuBuffer m, IGpuBuffer o, float fv, int sz) { if (!_kernelCache.TryGetValue("masked_fill_kernel",out var k)) throw new InvalidOperationException("CUDA kernel not found: masked_fill_kernel"); using var _=PushContext(); IntPtr ip=i.Handle,mp=m.Handle,op=o.Handle; void** a=stackalloc void*[5]; a[0]=&ip;a[1]=&mp;a[2]=&op;a[3]=&fv;a[4]=&sz; LaunchKernel(k,(uint)((sz+DefaultBlockSize-1)/DefaultBlockSize),DefaultBlockSize,a); }
    public unsafe void IndexSelect(IGpuBuffer i, IGpuBuffer idx, IGpuBuffer o, int ni, int isz) { if (!_kernelCache.TryGetValue("index_select",out var k)) throw new InvalidOperationException("CUDA kernel not found: index_select"); using var _=PushContext(); IntPtr ip=i.Handle,idxp=idx.Handle,op=o.Handle; void** a=stackalloc void*[5]; a[0]=&ip;a[1]=&idxp;a[2]=&op;a[3]=&ni;a[4]=&isz; uint t=(uint)(ni*isz); LaunchKernel(k,(t+DefaultBlockSize-1)/DefaultBlockSize,DefaultBlockSize,a); }

    public unsafe void CrossEntropyLoss(IGpuBuffer p, IGpuBuffer t, IGpuBuffer l, int bs, int nc) { if (!_kernelCache.TryGetValue("cross_entropy_loss",out var k)) throw new InvalidOperationException("CUDA kernel not found: cross_entropy_loss"); using var _=PushContext(); IntPtr pp=p.Handle,tp=t.Handle,lp=l.Handle; void** a=stackalloc void*[5]; a[0]=&pp;a[1]=&tp;a[2]=&lp;a[3]=&bs;a[4]=&nc; LaunchKernel(k,(uint)((bs+DefaultBlockSize-1)/DefaultBlockSize),DefaultBlockSize,a); }
    public unsafe void MseLoss(IGpuBuffer p, IGpuBuffer t, IGpuBuffer l, int bs, int nf) { if (!_kernelCache.TryGetValue("mse_loss",out var k)) throw new InvalidOperationException("CUDA kernel not found: mse_loss"); using var _=PushContext(); IntPtr pp=p.Handle,tp=t.Handle,lp=l.Handle; void** a=stackalloc void*[5]; a[0]=&pp;a[1]=&tp;a[2]=&lp;a[3]=&bs;a[4]=&nf; LaunchKernel(k,(uint)((bs+DefaultBlockSize-1)/DefaultBlockSize),DefaultBlockSize,a); }
    public void BceLoss(IGpuBuffer p, IGpuBuffer t, IGpuBuffer l, int sz) => LaunchFusedBinary("bce_loss",p,t,l,sz);
    public unsafe void DropoutMask(IGpuBuffer m, int sz, float kp, ulong seed) { if (!_kernelCache.TryGetValue("dropout_mask",out var k)) throw new InvalidOperationException("CUDA kernel not found: dropout_mask"); using var _=PushContext(); IntPtr mp=m.Handle; void** a=stackalloc void*[4]; a[0]=&mp;a[1]=&sz;a[2]=&kp;a[3]=&seed; LaunchKernel(k,(uint)((sz+DefaultBlockSize-1)/DefaultBlockSize),DefaultBlockSize,a); }
    public unsafe void GaussianNoise(IGpuBuffer o, int sz, float mn, float sd, ulong seed) { if (!_kernelCache.TryGetValue("gaussian_noise",out var k)) throw new InvalidOperationException("CUDA kernel not found: gaussian_noise"); using var _=PushContext(); IntPtr op=o.Handle; void** a=stackalloc void*[5]; a[0]=&op;a[1]=&sz;a[2]=&mn;a[3]=&sd;a[4]=&seed; LaunchKernel(k,(uint)((sz+DefaultBlockSize-1)/DefaultBlockSize),DefaultBlockSize,a); }

    public void LogSoftmax(IGpuBuffer i, IGpuBuffer o, int os, int isz) => LaunchFusedAxis("log_softmax",i,o,os,isz);
    public unsafe void GumbelSoftmax(IGpuBuffer i, IGpuBuffer o, int os, int isz, float temp, ulong seed) { if (!_kernelCache.TryGetValue("gumbel_softmax",out var k)) throw new InvalidOperationException("CUDA kernel not found: gumbel_softmax"); using var _=PushContext(); IntPtr ip=i.Handle,op=o.Handle; void** a=stackalloc void*[6]; a[0]=&ip;a[1]=&op;a[2]=&os;a[3]=&isz;a[4]=&temp;a[5]=&seed; LaunchKernel(k,(uint)((os+DefaultBlockSize-1)/DefaultBlockSize),DefaultBlockSize,a); }
    public void Sparsemax(IGpuBuffer i, IGpuBuffer o, int os, int isz) => LaunchFusedAxis("sparsemax",i,o,os,isz);
    public void TaylorSoftmax(IGpuBuffer i, IGpuBuffer o, int os, int isz) => LaunchFusedAxis("taylor_softmax",i,o,os,isz);
    public void SphericalSoftmax(IGpuBuffer i, IGpuBuffer o, int os, int isz) => LaunchFusedAxis("spherical_softmax",i,o,os,isz);
    public unsafe void BatchDotProduct(IGpuBuffer a1, IGpuBuffer b1, IGpuBuffer o, int bs, int dim) { if (!_kernelCache.TryGetValue("batch_dot_product",out var k)) throw new InvalidOperationException("CUDA kernel not found: batch_dot_product"); using var _=PushContext(); IntPtr ap=a1.Handle,bp=b1.Handle,op=o.Handle; void** a=stackalloc void*[5]; a[0]=&ap;a[1]=&bp;a[2]=&op;a[3]=&bs;a[4]=&dim; LaunchKernel(k,(uint)((bs+DefaultBlockSize-1)/DefaultBlockSize),DefaultBlockSize,a); }
    public unsafe void OuterProduct(IGpuBuffer a1, IGpuBuffer b1, IGpuBuffer o, int M, int N) { if (!_kernelCache.TryGetValue("outer_product",out var k)) throw new InvalidOperationException("CUDA kernel not found: outer_product"); using var _=PushContext(); IntPtr ap=a1.Handle,bp=b1.Handle,op=o.Handle; void** a=stackalloc void*[5]; a[0]=&ap;a[1]=&bp;a[2]=&op;a[3]=&M;a[4]=&N; uint t=(uint)(M*N); LaunchKernel(k,(t+DefaultBlockSize-1)/DefaultBlockSize,DefaultBlockSize,a); }
    public unsafe void BatchOuterProduct(IGpuBuffer a1, IGpuBuffer b1, IGpuBuffer o, int bs, int M, int N) { if (!_kernelCache.TryGetValue("batch_outer_product",out var k)) throw new InvalidOperationException("CUDA kernel not found: batch_outer_product"); using var _=PushContext(); IntPtr ap=a1.Handle,bp=b1.Handle,op=o.Handle; void** a=stackalloc void*[6]; a[0]=&ap;a[1]=&bp;a[2]=&op;a[3]=&bs;a[4]=&M;a[5]=&N; uint t=(uint)(bs*M*N); LaunchKernel(k,(t+DefaultBlockSize-1)/DefaultBlockSize,DefaultBlockSize,a); }
    public unsafe void CosineSimilarity(IGpuBuffer a1, IGpuBuffer b1, IGpuBuffer o, int bs, int dim) { if (!_kernelCache.TryGetValue("cosine_similarity",out var k)) throw new InvalidOperationException("CUDA kernel not found: cosine_similarity"); using var _=PushContext(); IntPtr ap=a1.Handle,bp=b1.Handle,op=o.Handle; void** a=stackalloc void*[5]; a[0]=&ap;a[1]=&bp;a[2]=&op;a[3]=&bs;a[4]=&dim; LaunchKernel(k,(uint)((bs+DefaultBlockSize-1)/DefaultBlockSize),DefaultBlockSize,a); }
    public unsafe void PairwiseDistance(IGpuBuffer a1, IGpuBuffer b1, IGpuBuffer o, int M, int N, int dim) { if (!_kernelCache.TryGetValue("pairwise_distance",out var k)) throw new InvalidOperationException("CUDA kernel not found: pairwise_distance"); using var _=PushContext(); IntPtr ap=a1.Handle,bp=b1.Handle,op=o.Handle; void** a=stackalloc void*[6]; a[0]=&ap;a[1]=&bp;a[2]=&op;a[3]=&M;a[4]=&N;a[5]=&dim; uint t=(uint)(M*N); LaunchKernel(k,(t+DefaultBlockSize-1)/DefaultBlockSize,DefaultBlockSize,a); }
    public unsafe void PairwiseDistanceSquared(IGpuBuffer a1, IGpuBuffer b1, IGpuBuffer o, int M, int N, int dim) { if (!_kernelCache.TryGetValue("pairwise_distance_squared",out var k)) throw new InvalidOperationException("CUDA kernel not found: pairwise_distance_squared"); using var _=PushContext(); IntPtr ap=a1.Handle,bp=b1.Handle,op=o.Handle; void** a=stackalloc void*[6]; a[0]=&ap;a[1]=&bp;a[2]=&op;a[3]=&M;a[4]=&N;a[5]=&dim; uint t=(uint)(M*N); LaunchKernel(k,(t+DefaultBlockSize-1)/DefaultBlockSize,DefaultBlockSize,a); }
