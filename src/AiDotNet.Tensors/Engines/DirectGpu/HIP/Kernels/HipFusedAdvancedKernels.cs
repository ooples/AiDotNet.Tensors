    ...
    float alpha_bar_t_minus_1)
{
    ...
    // Defence in depth: host-side rejects alpha_bar_t <= 0, but if a
    // regression let it through, sqrtf(0) makes c_xt and c_eps Inf and
    // poisons every output element. Clamp to a tiny positive value.
