...
public sealed class CudaGraphScope : IDisposable
{
    ...
    /// <summary>
    /// Gets whether the backend supports graph capture.
    /// </summary>
    public bool IsSupported => _backend is IAsyncGpuBackend { SupportsGraphCapture: true };
    ...
    public CudaGraphScope(IGpuBatchExecution backend)
    {
    ...
    /// All subsequent GPU operations on this backend will be recorded instead of executed.
    /// </summary>
    ...
    /// The graph must have been captured first via BeginCapture/EndCapture.
    /// </summary>
    public void Replay()
    {
        if (!_hasGraph)
        ...
        // Use the batch execution's replay mechanism
        _backend.BeginBatch();
        _backend.EndBatch();
    }
