...
public class DelegatingGpuBackend : IDirectGpuBackend
{
    ...
    public virtual void VarBackward(IGpuBuffer gradOutput, IGpuBuffer input, IGpuBuffer mean, IGpuBuffer gradInput, int outerSize, int reduceSize) => Inner.VarBackward(gradOutput, input, mean, gradInput, outerSize, reduceSize);
    public virtual void StdBackward(IGpuBuffer gradOutput, IGpuBuffer input, IGpuBuffer mean, IGpuBuffer std, IGpuBuffer gradInput, int outerSize, int reduceSize) => Inner.StdBackward(gradOutput, input, mean, std, gradInput, outerSize, reduceSize);
    public virtual void MaskedFillBackward(IGpuBuffer gradOutput, IGpuBuffer mask, IGpuBuffer gradInput, int size) => Inner.MaskedFillBackward(gradOutput, mask, gradInput, size);
    public virtual void WhereBackward(IGpuBuffer gradOutput, IGpuBuffer condition, IGpuBuffer gradX, IGpuBuffer gradY, int size) => Inner.WhereBackward(gradOutput, condition, gradX, gradY, size);
    public virtual void NormBackward(IGpuBuffer gradOutput, IGpuBuffer input, IGpuBuffer norm, IGpuBuffer gradInput, int outerSize, int reduceSize) => Inner.NormBackward(gradOutput, input, norm, gradInput, outerSize, reduceSize);
    public virtual void LogSumExpBackward(IGpuBuffer gradOutput, IGpuBuffer input, IGpuBuffer lse, IGpuBuffer gradInput, int outerSize, int reduceSize) => Inner.LogSumExpBackward(gradOutput, input, lse, gradInput, outerSize, reduceSize);
    public virtual void AvgPool1D(IGpuBuffer input, IGpuBuffer output, int batch, int channels, int inLength, int outLength, int kernelSize, int stride) => Inner.AvgPool1D(input, output, batch, channels, inLength, outLength, kernelSize, stride);
    public virtual void MaxPool1D(IGpuBuffer input, IGpuBuffer output, int batch, int channels, int inLength, int outLength, int kernelSize, int stride) => Inner.MaxPool1D(input, output, batch, channels, inLength, outLength, kernelSize, stride);
    public virtual void BilinearUpsample2D(IGpuBuffer input, IGpuBuffer output, int batch, int channels, int inH, int inW, int outH, int outW) => Inner.BilinearUpsample2D(input, output, batch, channels, inH, inW, outH, outW);
    public virtual void ScatterMean(IGpuBuffer source, IGpuBuffer indices, IGpuBuffer output, IGpuBuffer counts, int sourceSize, int outputSize, int featureSize) => Inner.ScatterMean(source, indices, output, counts, sourceSize, outputSize, featureSize);
    ...
    // StopGradient, Fused Linear, and IoU Operations — delegate to inner backend
    public virtual void CopyBuffer(IGpuBuffer source, IGpuBuffer destination, int size) => Inner.CopyBuffer(source, destination, size);
    public virtual void FusedLinearReLU(IGpuBuffer input, IGpuBuffer weight, IGpuBuffer bias, IGpuBuffer output, int batchSize, int inFeatures, int outFeatures) => Inner.FusedLinearReLU(input, weight, bias, output, batchSize, inFeatures, outFeatures);
    public virtual void FusedLinearSigmoid(IGpuBuffer input, IGpuBuffer weight, IGpuBuffer bias, IGpuBuffer output, int batchSize, int inFeatures, int outFeatures) => Inner.FusedLinearSigmoid(input, weight, bias, output, batchSize, inFeatures, outFeatures);
    public virtual void FusedLinearTanh(IGpuBuffer input, IGpuBuffer weight, IGpuBuffer bias, IGpuBuffer output, int batchSize, int inFeatures, int outFeatures) => Inner.FusedLinearTanh(input, weight, bias, output, batchSize, inFeatures, outFeatures);
    public virtual void FusedLinearGELU(IGpuBuffer input, IGpuBuffer weight, IGpuBuffer bias, IGpuBuffer output, int batchSize, int inFeatures, int outFeatures) => Inner.FusedLinearGELU(input, weight, bias, output, batchSize, inFeatures, outFeatures);
    public virtual void FusedLinearSwish(IGpuBuffer input, IGpuBuffer weight, IGpuBuffer bias, IGpuBuffer output, int batchSize, int inFeatures, int outFeatures) => Inner.FusedLinearSwish(input, weight, bias, output, batchSize, inFeatures, outFeatures);
    public virtual void FusedLinearReLUBackward(IGpuBuffer gradOutput, IGpuBuffer input, IGpuBuffer weight, IGpuBuffer preActivation, IGpuBuffer gradInput, IGpuBuffer gradWeight, IGpuBuffer gradBias, int batchSize, int inFeatures, int outFeatures) => Inner.FusedLinearReLUBackward(gradOutput, input, weight, preActivation, gradInput, gradWeight, gradBias, batchSize, inFeatures, outFeatures);
    public virtual void FusedLinearSigmoidBackward(IGpuBuffer gradOutput, IGpuBuffer input, IGpuBuffer weight, IGpuBuffer output, IGpuBuffer gradInput, IGpuBuffer gradWeight, IGpuBuffer gradBias, int batchSize, int inFeatures, int outFeatures) => Inner.FusedLinearSigmoidBackward(gradOutput, input, weight, output, gradInput, gradWeight, gradBias, batchSize, inFeatures, outFeatures);
    public virtual void FusedLinearTanhBackward(IGpuBuffer gradOutput, IGpuBuffer input, IGpuBuffer weight, IGpuBuffer output, IGpuBuffer gradInput, IGpuBuffer gradWeight, IGpuBuffer gradBias, int batchSize, int inFeatures, int outFeatures) => Inner.FusedLinearTanhBackward(gradOutput, input, weight, output, gradInput, gradWeight, gradBias, batchSize, inFeatures, outFeatures);
    public virtual void FusedLinearGELUBackward(IGpuBuffer gradOutput, IGpuBuffer input, IGpuBuffer weight, IGpuBuffer preActivation, IGpuBuffer gradInput, IGpuBuffer gradWeight, IGpuBuffer gradBias, int batchSize, int inFeatures, int outFeatures) => Inner.FusedLinearGELUBackward(gradOutput, input, weight, preActivation, gradInput, gradWeight, gradBias, batchSize, inFeatures, outFeatures);
    public virtual void FusedLinearSwishBackward(IGpuBuffer gradOutput, IGpuBuffer input, IGpuBuffer weight, IGpuBuffer preActivation, IGpuBuffer gradInput, IGpuBuffer gradWeight, IGpuBuffer gradBias, int batchSize, int inFeatures, int outFeatures) => Inner.FusedLinearSwishBackward(gradOutput, input, weight, preActivation, gradInput, gradWeight, gradBias, batchSize, inFeatures, outFeatures);
    public virtual void IoULoss(IGpuBuffer predicted, IGpuBuffer target, IGpuBuffer loss, int numBoxes) => Inner.IoULoss(predicted, target, loss, numBoxes);
    public virtual void GIoULoss(IGpuBuffer predicted, IGpuBuffer target, IGpuBuffer loss, int numBoxes) => Inner.GIoULoss(predicted, target, loss, numBoxes);
    public virtual void DIoULoss(IGpuBuffer predicted, IGpuBuffer target, IGpuBuffer loss, int numBoxes) => Inner.DIoULoss(predicted, target, loss, numBoxes);
    public virtual void CIoULoss(IGpuBuffer predicted, IGpuBuffer target, IGpuBuffer loss, int numBoxes) => Inner.CIoULoss(predicted, target, loss, numBoxes);
    public virtual void IoULossBackward(IGpuBuffer gradOutput, IGpuBuffer predicted, IGpuBuffer target, IGpuBuffer gradPredicted, int numBoxes) => Inner.IoULossBackward(gradOutput, predicted, target, gradPredicted, numBoxes);
